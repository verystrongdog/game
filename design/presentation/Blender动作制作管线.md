...
（胶囊半径 0.3117 + 坐面半深 0.21），而顶杆还在坐面**另一侧** 0.44 m，合计 0.44+0.52 远超
...
### 实测读数（2026-09-14，Blender 5.1.2）

| 帧 | 弯腰 | 左手（腕） | 中指尖 | 头高 | 脚漂移 | 肘角 |
|---|---|---|---|---|---|---|
| 1 | 0° | `(0.713, 0.055, 1.441)` | — | 1.599 | 0.0 mm | 180° |
| 21 | 30° | `(0.167, -0.409, 0.929)` | `(0.173, -0.533, 0.746)` | 1.510 | 0.0 mm | — |
| 31 | 45° | `(0.169, -0.460, 0.856)` | `(0.176, -0.577, 0.668)` | 1.415 | 0.0 mm | — |
| **41** | **55°** | **`(0.170, -0.470, 0.840)`** = 目标 | `(0.249, -0.474, 0.785)` | 1.337 | 0.0 mm | **133.2°** |

- 顶杆位置：`y = -0.470`（靠背平面）· `z = 0.7949`（离地）· 抓点 `x = ±0.17`（靠背宽 0.42）。
- **腕在顶杆上方 45 mm、指尖在顶杆平面之前且低于顶面** ⇒ 是**手指绕过去扣住**，不是指尖蹭到。
- 每个关键帧上**脚相对静止位漂移 0.0 mm**（临时 IK 把双脚钉在原地）。
- 导出：`BendGripChairBack.fbx` · E0–E6 **全绿**（烘焙保真 0.001 mm · 回导 0.0015 mm · E5 端点 0.153 mm）。
- Unity：`clip.humanMotion=True` · `len=2.0000` · `avatar isValid/isHuman` · `animationType=3` · `avatarSetup=1` · 三条告警串空。

### 手指怎么扣的（母版没有手指控制骨）

13 根控制骨里没有手指 ⇒ **直接在手指变形骨上打键**（导出烘焙覆盖全部变形骨，故能带出去）。
**蜷曲轴由脚本自测**（试 X/Z 两轴 × 两符号，取"指尖朝掌心"分量最大者）：实测
**Z 轴、符号 −1，指尖朝掌心 173 mm**（左右手同解）。⇒ 不靠猜，换骨架/换手重跑即可。

### ⚠️ 三条实测坑（全部由 E5/E6 抓出来）

1. **导出物的 bind pose = 导出那一刻的当前帧姿势**（详见危险点表 §四）。本动作首帧原本不是中立姿势，
   E5 报出 **69 m** 偏差；而**探针动作侥幸通过**只因它末帧恰好是静止 ⇒ 这条以前一直在裸奔。
2. `armature.pose_position = "REST"` 是**错误修法**：bind pose 对了，但动画被压成常量（E6 偏差 = 整段幅度）。
3. `bpy.data.actions.new()` 同名静默建 `.001`，下游按名取到**旧动作**（改完动作导出结果一模一样）。

### 未登记动作词条（诚实清单）

本动作是**动画素材**，**没有**进 `data/action_set.json` / `ActionCatalog` / `ActionIds`：
动作词表是**受控**的（[动作库规格](%E5%8A%A8%E4%BD%9C%E5%BA%93%E8%A7%84%E6%A0%BC.md) §二），
要不要新增词条属设计裁定。另外它属"手里有东西"那条**已由 owner 暂停**的线的**邻域**
（本动作只做**姿势**，不涉及 §戊·2 的挂点/物理机制）。
