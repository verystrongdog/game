...
| 3 | **母版写回** | 创作脚本**默认 `save_as_mainfile(filepath=tmpl)`**，而 `tmpl` 的 argparse 默认值**就是母版**（`.scratch/blender_assets/xbot/XBot_AnimationTemplate.blend`）；实测**5 处**（`code/tools/author_xbot_chair_grab.py` `:1908 / :3729 / :4859 / :5635 / :6801`） | `--host live` 分支**不写盘**（预览，改动只在会话内存）· `--no-save` 只解算不写 · [危险点表](../engineering/危险点表.md) §四「多实例 lab 与母版 / 导出源」行已登记"不写 `--no-save` 就会**静默污染母版**" | `--force`（`:143`）**声明了却零消费** ⇒ 覆盖前**没有确认**⇒ 门禁要求：lab / 预览一律 `--no-save` · 母版是**共享回归基准**（V2 / V3 / V9 硬判 65 骨 / 单一根 / 骨集合），写回前后核**母版自检退出码与 `sha256`**（⚠️ 母版在 `.scratch/`、被 `.gitignore` 排除 ⇒ `git status` **看不见**它被动过，别拿它当判据）· 正式动作**另存副本**，不直接写基准 |
...
**复核中登记的"与文档不符"四条**——owner 2026-09-16 裁：**只登记、不开 issue**；本件**不改 `code/tools/**`**
