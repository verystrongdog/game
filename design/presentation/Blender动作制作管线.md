...
> 🔧 **修正注记（2026-09-15，按[项目规约 §五 第 4 条](../conventions/README.md)「口语判词三问」逐词走完，owner 裁决）**：上一行里三个宾语原**都没有定义**，因此那句禁令**既不可执行也不可违反**。逐项处置如下（三项的注册表条目见 `data/term_registry.json`）：
>
> | 宾语 | 在本条里的处置 | 定义所在 |
> |---|---|---|
> | **脚滑** | **问 1（可落成量）**——禁令**成立**，判据落在**代理量**上 | [动画处理能力对照实验](动画处理能力对照实验.md) §三 **M5**：接触帧 → `t_c+1` 帧之间、足底最低点（足骨 y 与趾骨 y 的**较小者**）的水平位移。⚠️ **前置 = 接触相位识别**（🔧 2026-09-15 **已建** = [#165](https://github.com/verystrongdog/game/issues/165)；**本条读数尚未接它**），现有读数是在 lab 里**手选 `t_c`** 算的 |
> | **重心** | **问 1（可落成量）**——禁令**成立**，但判的是**代理点**而非力学质心 | 本仓定义 = `mixamorig:Hips` 骨**世界位置的水平投影**（**零常量代理点，不含质量模型**）。配套量「支撑多边形」的前置同为接触相位 ⇒ 判据部分按[口径 §8.3](动作描述口径.md) 过渡态**只报读数、由 owner 目视判** |
> | **审美** | **问 2（仅目视语汇）**——**本项不构成禁令** | **无可计算定义**。本仓结构性约束：**agent 无图像输入**（见本文 §八·3）⇒ 它不可能成为 agent 侧判据，只能作为 owner 的目视意见；其合法入口是[口径 §二](动作描述口径.md) 的「**只能发起讨论，不能结案**」——须由 agent 翻成 ① 点约束或 ② 关系判据才算数 |
>
> ⇒ 「不做自动修复」这句话此后**逐项可判**：前两项有代理量可读，第三项不再是能力面而是**输入通道**。本注记只改这句话的**可执行性**，不改它的语义（本条仍不做任何自动修复）。
...
| ⚠️ 探针**观感**（实测几何） | 关键帧绕的是**骨的局部 X 轴**，在这套 Mixamo 骨轴上那是**竖直面内的抬摆**而非前后摆：实测左手**相对肩 −0.1824 → +0.4450 m**（摆到过头）；双脚 y 只在 **0.0868–0.1496 m**（**贴地前刮、几乎不抬脚**）；骨盆只做起落。合起来的观感**类似走钢丝**——这是覆盖性探针的必然长相，不是缺陷（审美与脚滑在本条「明确排除」内；owner 2026-09-14 目视确认为「没有发现明显问题」；🔧 两个词的逐项处置见 §一 的 2026-09-15 修正注记——`脚滑` 有代理量 M5、`审美` **不构成禁令**） | Play 内逐帧读数，见本文 §六·3 |
...
#### 2.1.4 无约束变形骨的轴向语义（实测 2026-09-14，[#161](https://github.com/verystrongdog/game/issues/161)）

§2.1.1 只覆盖 **13 根控制骨**。母版里另外 **52 根变形骨没有约束**（脊柱 3 · 颈/头/头顶 3 · 肩 2 · 手指 40 · 脚趾 4），
其中**除 40 根手指之外**的 **12 根**在下面。没有这张表时，owner 说"**目视前方**"这类话，
agent 手上**没有轴—位移映射**——而这正是[口径](%E5%8A%A8%E4%BD%9C%E6%8F%8F%E8%BF%B0%E5%8F%A3%E5%BE%84.md) §十 V-b 里"旧判据不可能报出"的一项。

**口径**：绕该骨**局部轴** +20°（基准**先归零**，§7.4），量四处——本骨骨尖 · 链末标志点 · 偏轴标志点 ·
**蒙皮最大位移**（`Beta_Surface`/`Beta_Joints` 取大者）。方向分解为**体侧 / 前后 / 上下**（正号 = 向**左手侧** / 向**前** / 向**上**）。
**可复现**：`blender --background --factory-startup --python-exit-code 1 --python code/tools/measure_xbot_control_axes.py -- --group deform --template <母版> --report <json>`

⚠️ **为什么比 §2.1.1 多一列"蒙皮"**：头 / 颈 / 趾这类骨的扭转**在任何骨尖上都看不见**（下游骨与本骨共线，
叶骨干脆没有下游）。只量骨尖会得出"这根轴没用"这个**错误**结论。实测三态分布（36 格）：
**可观察 31 · 仅蒙皮可见 2 · 不可观察 3**。

| 骨 | 局部 X +20° | 局部 Y +20° | 局部 Z +20° |
|---|---|---|---|
| `Spine` | **前倾**（头顶 前+302.6 / 上−86.7 = **314.8 mm**） | **沿脊柱轴扭转**（本骨骨尖 **0**；左手侧摆 **276.5 mm**；蒙皮 **313.6 mm**） | **侧倾**（头顶 侧−305.8 = **310.5 mm**） |
| `Spine1` | 前倾（头顶 280.5 mm） | 扭转（**与 `Spine` 逐值相同**——三根脊柱共线，绕同一轴转同一角 ⇒ 刚性同转） | 侧倾（275.8 mm） |
| `Spine2` | 前倾（248.8 mm） | 扭转（同上，逐值相同） | 侧倾（244.7 mm） |
| `Neck` | **点头/抬头**（头顶 前+177.9 / 上−70.7 = **191.5 mm**） | **转头**（链末 45.0；蒙皮 侧+60.3 = **61.3 mm**） | **歪头**（侧−183.3 = **186.1 mm**） |
| `Head` | 点头/抬头（**157.6 mm**） | 转头（蒙皮 侧+50.6 = **51.4 mm**） | 歪头（**153.7 mm**） |
| `HeadTop_End` | ⚠️ 只动**它自己的骨尖**（80.8 mm）· 蒙皮 **0.00** | ⛔ **不可观察**（叶骨 + 骨尖在轴上） | ⚠️ 同 X（153.7→80.8 mm，仅自身骨尖） |
| `LeftShoulder` | **落臂**（左手 上−251.9 = **256.5 mm**） | **沿臂轴扭转**（47.3 mm 上−47.3） | **向前摆臂**（前+256.8 = **260.3 mm**） |
| `RightShoulder` | 落臂（256.5 mm；**侧符号与左相反**） | 沿臂轴扭转（47.3 mm **上+45.0**——与左**反号**） | 向前摆臂（**前−253.7**——与左**反号**） |
| `LeftToeBase` | **抬脚趾/勾脚尖**（趾尖 上+64.2 = **64.4 mm**） | ⚠️ **仅蒙皮可见**（骨尖 0 / 标志点 0；蒙皮 **22.28 mm**） | 脚趾侧摆（趾尖 侧+63.5 = **64.4 mm**） |
| `RightToeBase` | 抬脚趾（64.4 mm） | ⚠️ 仅蒙皮可见（22.28 mm） | 脚趾侧摆（**侧+63.5**——**与左同号**） |
| `LeftToe_End` / `RightToe_End` | ⚠️ 只动自身骨尖（32.2 mm）· 蒙皮 0.00 | ⛔ **不可观察** | ⚠️ 同 X |

**四条读表须知**：

1. **`HeadTop_End` 与 `Toe_End` 是"骨尖标记"，不是关节**——它们是**叶骨**（无子骨）、且**没有顶点权重**（蒙皮位移 0.00）。
   转它们只会移动一个**虚拟端点**，皮肤一动不动。⇒ **不要拿它们当判据**，也不要在需求里指它们。
2. **肩的左右符号相反、脚趾的左右符号相同**——这是对 §2.1.2 已实测符号规则的**独立复现**（臂镜像 / 腿同轴）。
3. **三根脊柱的扭转读数逐值相同**不是 bug：它们共线，绕同一条轴转同一个角 ⇒ 上半身刚性同转。
   ⇒ 想"扭腰"时**改哪一根都一样**，不必叠乘。
4. **"蒙皮最大位移"取的是全网格最大点**（通常是手/指尖），不是该骨附近的表面——它回答"这根轴能不能被看见"，
   **不回答**"该骨附近动了多少"。骨附近的量请用本骨骨尖 / 链末 / 偏轴三列。

> 测量卫生：蒙皮求值**噪声底实测 0.000000 mm**（同一姿势重求值两次逐值相同）⇒ "动了没动"按 **> 0** 判，
> **不设人为阈值**；两遍独立进程运行 **806 个可比数值逐值相同**、打印表逐字相同。原始读数见
> [证据](../engineering/evidence/action-description-deform-axes-2026-09-14.md)。
> ⚠️ 手部 40 根手指骨**不在本表**——上一轮的屈曲轴是脚本自测的，且其**符号存在待裁定分歧**（见 [#160](https://github.com/verystrongdog/game/issues/160) 证据 §六.2，转 [#163](https://github.com/verystrongdog/game/issues/163)）。

#### 2.1.5 身体分区索引（19 分区 · 🔧 2026-09-16）

**本节只补管线侧的三条硬规矩**；分区**归属与缺口**的权威是 [动作描述口径](%E5%8A%A8%E4%BD%9C%E6%8F%8F%E8%BF%B0%E5%8F%A3%E5%BE%84.md) **§十五**，轴表正本仍是上面的 §2.1.1（13 控制骨）与 §2.1.4（12 无约束变形骨）——**本节不复制任何位移读数**（先例：两处各算一份椅子几何、差 **0.46 m** ⇒ "残差 0.0 mm"全绿而**手离椅背 440 mm**）。

粒度 = **肢段级 19 分区 + 手 / 脚子分区**（owner 2026-09-16 裁定；手部子分区 12 格见口径 §13.2）。

| # | 规矩 | 依据 |
|---|---|---|
| **1** | **13 根标记骨不许进任何"部位"**——`HeadTop_End` · `LeftToe_End`/`RightToe_End` · 手指 `…4` ×10（**推导**：无子骨 **且** 零蒙皮权重 **且** 不在控制集合）。它们是**骨尖标记不是关节**：**蒙皮位移 0.00**，转它们**皮肤一动不动** | §2.1.4 注 1 已写"**不要拿它们当判据**"；本行把它扩到**需求面**——owner 说"头顶 / 脚尖"时也**不许落上去**，否则判据**平凡全绿而画面不动**。⚠️ **不能删**（V2 与导出 E2/E3 要恰好 **65** 根变形骨）⇒ 只能在预览层隐藏（`HIDDEN_MARKERS`；实测隐藏后 E0–E6 重跑全绿） |
| **2** | **写面二分**：**13 个分区**归 `CTRL_*`（**只动控制骨**——直接转 `mixamorig:*` 会被约束覆盖）；**脊柱 / 颈头 / 足趾 / 手指**这 6 个分区**无控制骨**，只能**直接打键** | §2.1 步 3（"转了个寂寞"）· §2.1.4（52 根无约束）。⚠️ 肩这一分区**两个写面都有**（`CTRL_*Arm` + 无约束的 `*Shoulder`） |
| **3** | **多实例 lab（三件同屏）**：① lab **不是导出源**——`code/tools/export_xbot_action.py` 硬要求「**恰好 1 个 Armature**」② lab **不写回母版**（否则 V2/V3/V9 从此失败）③ 复制实例后指派 action **必须走取道层** `code/tools/blender_action_compat.py` 的 `assign_action()`——5.x 侧**还要设 `action_slot`**，否则那条 action **不驱动任何东西** ④ 可见性收敛（只留 `MIXAMORIG` + 隐藏 13 根标记骨）**逐实例各跑一次** | 口径 §15.8（间距 **5.0 m**，owner 圈定；lab 落 `.scratch/` + `--no-save`）· [危险点表](../engineering/%E5%8D%B1%E9%99%A9%E7%82%B9%E8%A1%A8.md) §四 |
...
### 7.8 成熟度边界与风险门禁（2026-09-16 · [#173](https://github.com/verystrongdog/game/issues/173)）

> **决策（owner，2026-09-16）：选 C —— 分层成熟度 + 风险门禁。**
> **定位：当前能力 = 「X Bot 单机、单写者、人工目视参与的受控动画研发管线」，不是通用自然语言动画生产系统。**
> 选项 **A**（视为已可普遍使用的自然语言→动画系统）与 **B**（暂停动作探索、先解决全部安全 / 事务 / 通用化 / Unity 准入）**均不采纳**。
> 本决策**只限定成熟度与准入**：#173 的预期差分**只允许改本文**；`code/tools/**`、活体桥、`.blend` / FBX / `.anim` / `.meta` / Animator 资产**一字未动**；四轴状态（[切片 §二](../slices/CP-01-ward-1f/slice.md)）与 [PLAYABLE](../../PLAYABLE.md) 不动。

**为什么是 C**（两条理由都对着读数，不是口味）

| 排除项 | 理由（实测） |
|---|---|
| **A** | 现有证据只支持"受控样本可行"：**真实动作（#163 / #164 / #166 / #169 / #170 / #171 六件）没有一件走完准入阶梯**——走到 `UNITY_PLAY_VALIDATED` 的**只有母版探针**，`RUNTIME_CONNECTED` 实测为 **0**（探针 FBX 的 guid 在 `code/unity/Assets/` 内**除自身 `.meta` 外零引用** · `code/src` 的动画引用 **0 处** · `data/action_set.json` 17 词条**无一条**引 Blender 产物）；且"机械全绿"与 owner 目视背离**已登记四次**（见"分层纪律 2"）。 |
| **B** | 下表 10 项风险里 **6 项已有实测护栏**（"现有护栏"列），把它们与"零护栏"的四项捆成一个总前置，等于让最保守那项决定全部节奏；且桥当前只在**本机 + 单写者 + 人工在场**条件下使用（`bb-launch.sh` 探活复用 + 随机 token 门禁），尚未到达 RFC 所说的"暴露到非本机 / 多人并发 / 处理不可恢复资产"门槛。 |

**准入阶梯（七态）**——状态名取自 #173 选项 C；**进入判据一律引用已有件与已有容差（零新阈值）**

| 状态 | 进入判据 | 现状（2026-09-16 实测） |
|---|---|---|
| `DRAFT` | 宿主里有动作名 + 帧范围 + 自由度表（[口径](动作描述口径.md) §五），尚无读数 | — |
| `BLENDER_VISUAL_ACCEPTED` | owner 经**活体窗口**目视：口径 §十 **V-c**（0 轮返工，或 ≤1 轮且该轮**能指名某个量**）。⚠️ 走满 **3 轮**才接受者**记通过、但 `动作描述口径` 的 `Health` 仍 `UNKNOWN`**（先例 = [#166](https://github.com/verystrongdog/game/issues/166) 收口记录） | #164 卡 1（第 2 轮「对，通过」）· #163（V-a…V-e 接受，附 3 条已知偏差）· #166（第 3 轮「可以了，现在没问题了」）· #169（3 轮后定稿，"抱头"那一解判不上）· #170（第 2 轮「确实后退了，但是太机械了」——后半句 owner 裁定不在本件范围）· #171（**0 轮返工**） |
...
| # | 风险 | 实测事实（来源） | 现有护栏 | 缺口 ⇒ 门禁要求 |
|---|---|---|---|---|
| 1 | **任意 Python 权限** | 桥「执行任意 Python，且**没有任何沙箱**」——等于交出那个 Blender 进程的全部能力（[桥 README](../../code/tools/blender-bridge/README.md) §四-1）；门禁只有一道 **token**（启动随机生成、写进状态文件） | token 门禁 · **默认只绑 `127.0.0.1`**（WSL 连不进来是有意的）· 状态文件已被 `.gitignore` 排除 | 一旦为连 WSL 显式设 `BB_HOST`，**同网段其他机器有 token 即可连**（本机最近一次桥会话的状态文件实测 `host = 172.25.48.1`，**非回环**）⇒ 门禁要求：只在**本机 + 人工在场**的会话里用 · 用完关窗口 · 状态文件不放共享盘 · **不暴露给第二消费者**（与 §7.7 中 C 的启动条件同一条口径） |
| 2 | **超时后继续执行** | 客户端 socket 超时 = 应用超时 **+30 s**，服务端改用**单调时钟死线**；**超时不取消在飞脚本**——「要真停就重起会话」（[#162 证据](../engineering/evidence/action-description-live-loop-2026-09-14.md) §六.6）；且超时后脚本跑完、结果留 pending（同 §六.1：**下一次请求"秒回"了上一次 67.6 s 任务的结果，且 `ok: true`**） | 把"超时"与"连不上"分开（`--timeout 20` 实测 19.7 s 返回 `{"error":"timeout"}` 并给出明确提示）· 长输出落 `bb-outbox/`（**48172 字符**不丢，旧代码只留尾部 20000） | **无取消通道**；**陈旧结果可能被误读成本次结果** ⇒ 门禁要求：活体任务的返回值须带**请求 ID 与母版身份**再判读 · 超时后的返回值**不得**当本次结果 · 要真停就重起会话 |
| 3 | **母版写回** | 创作脚本**默认 `save_as_mainfile(filepath=tmpl)`**，而 `tmpl` 的 argparse 默认值**就是母版**（`.scratch/blender_assets/xbot/XBot_AnimationTemplate.blend`）；实测**5 处**（`code/tools/author_xbot_chair_grab.py` `:1908 / :3729 / :4859 / :5635 / :6801`）。同链的**导出侧**另有一层：`export_xbot_action.py:157` 的默认输出落在**母版同目录**（`tmpl.parent / f"{action}.fbx"`），而 `:171-172` 只判"输出已存在又没给 `--force` 就拒绝"——**没有 `out_fbx == tmpl` 的相等性护栏**（全文件 9 处 `out_fbx` 无一处与 `tmpl` 比较）⇒ `--output <母版路径>` **不会被拒绝** | `--host live` 分支**不写盘**（预览，改动只在会话内存）· `--no-save` 只解算不写 · [危险点表](../engineering/危险点表.md) §四「多实例 lab 与母版 / 导出源」行已登记"不写 `--no-save` 就会**静默污染母版**" | `--force`（`:143`）**声明了却零消费** ⇒ 覆盖前**没有确认**；导出侧也**没有"不许指向输入"的判据** ⇒ 门禁要求：lab / 预览一律 `--no-save` · 母版是**共享回归基准**（V2 / V3 / V9 硬判 65 骨 / 单一根 / 骨集合），写回前后核**母版自检退出码与 `sha256`**（⚠️ 母版在 `.scratch/`、被 `.gitignore` 排除 ⇒ `git status` **看不见**它被动过，别拿它当判据——危险点表 §四 那行的判据列正写着 `git status`，见下表第 5 条）· 正式动作**另存副本**，不直接写基准 · **导出必须显式给 `--output`**，落点在 `Assets/` 或 `.scratch/` 产物区，**不得指向母版** |
| 4 | **错误会话** | 活体模式只检查**当前会话打开着某个文件**（`:1566`「`--host live` 要求母版已经在当前会话里打开」，7 个宿主各一处），**不校验这个文件就是目标母版**；`bb-launch.sh` 探活复用只查**桥是否响应**（`bpy.data.filepath` 回得来即复用），**不查已加载的是哪个文件**；另 #170 实测活体 `exec` 的**模块缓存**陈旧 | 状态文件里记了 `filepath`（人可核）· 后续按骨名取骨，取不到会报错（弱护栏） | 无会话身份断言 ⇒ 门禁要求：活体跑之前先读状态文件 `filepath` 并**与目标母版路径比对** · **一次会话只服务一件动作** · 改过脚本后**重起会话**再跑（模块缓存坑） |
| 5 | **同源生成 / 验收** | 生成器自判普遍存在：`V1–V9` / `E0–E6` / 宿主回显卡的 `ok`（`report["ok"]` + `problems`）**全由生成器自己判自己**；仓库自己登记的实例 = 「手指自测屈曲轴选到**侧弯** Z/−1」（见[危险点表](../engineering/危险点表.md) §四"手指蜷曲轴的选取"行）· [#164 证据](../engineering/evidence/hand-grip-cards-2026-09-15.md) §四自述判据 1「**不是独立验证——训练集测训练集**」· E5 的 bind pose 缺陷在探针动作上**一直没被拦住**（本文页脚第六次自述"探针一直裸奔侥幸过"） | 四个独立件（见分层纪律 3）+ `F4a` 逐帧持续型 + 时序零阈值 | 独立件**只判表 ↔ 算法一致**，`V1–V9` / `E0–E6` **无独立复核器** ⇒ 门禁要求：正式动作的判据**至少一条**出自**读产物**的独立进程（`evaluated_get(depsgraph)` 口径）· 生成器自判的判据**不得单独**作为通过证据 |
| 6 | **自然语言中间契约缺失** | 口径 §1.1 现走**甲**（owner 说人话 → agent 译成判据）；**乙**（程序吃结构化描述 / 约束集 schema）的三个触发器：① **未触发** · ② **已半触发**（`接触相位` 被 `重心` / `脚滑` / `悬空` 三条要求 ⇒ 以"件"的形态建）· ③ **未触发**；机器可读的只有 `data/hand_grip_cards.json`（卡 1 的**数值**权威）与 `data/action_set.json`（17 词条机器源）——**Motion Intent / 动作配方文件全仓检索 0 命中** | 三问骨架 + 回显卡 + 自由度表 + ε + 判据表（口径 §二…§十）把"人话"落成**可判据量**；`data/` 两个 `role: generator-input` 文件已进校验器 | "语言 → 参数"这一步**仍在人 / agent 头里** ⇒ 门禁要求：**在机器可读中间层出现、并由第二个不同动作验证之前，不得声明"自然语言可稳定产出游戏动画资产"**（泛化声明的前置；本件**不授权**建造该中间层） |
| 7 | **全帧接触 / 碰撞** | `M5`（脚滑代理量）量的是 `t_c → t_c + 1`**相邻两帧**（[对照实验](动画处理能力对照实验.md) §三 M5 · §2.2 辅助帧）；接触相位件**已建**（#165）但**现有 M5 读数仍未接它**（口径 §十·V-f 把 M5 列为 `持续`型的**例外**）；穿模判据只量**骨段**、碰撞体仅自身 / 地面 / 道具盒（口径 §十 表头自述"层差登记为已知偏差"）；全仓 `全帧` 检索 **0 命中** | `F4a` 持续型接触对**逐帧**（松一帧即红）· `接地` / `穿模` 两项持续型判据已进 CI | **没有逐顶点蒙皮层判据、没有穿墙判据**；骨点层与蒙皮层实测**最大层差 41 mm**（#165 证据 §七 #4，脚有俯仰时两口径给出**相反**答案）⇒ 门禁要求：凡"持续型"声明必须写明**逐帧区间** `[帧 a, 帧 b]` 与**取点层**；判据层**默认蒙皮层**（口径 §10.1 已裁） |
| 8 | **载体硬编码** | 宿主脚本 **6903 行**、`--task` 一个参数八种取值，`CTRL_` 命中 **63 处** · `X Bot` **4 处** · 椅子尺寸 / 位置是模块级常量；`data/hand_grip_cards.json` 的 `_meta.template` 固化了 **Windows 绝对路径** `Z:\home\dog\game\...`；仓库里**没有第二个载体的动作脚本**（`Y Bot.fbx` 虽在库，`code/tools/` 对它的引用 **0**） | 尺寸 / 位置写成常量并**逐条带来源**（如板厚 40 mm 基准格）· 判据里的容差全部**沿用**既有来源 | 换载体（Y Bot / 换道具）**整条链要重跑**（危险点表 §七 目标偏移行：产物**与目标载体绑定**）⇒ 门禁要求：状态声明必须写明**载体与道具**；换载体属**新件**，**不得沿用**旧读数 |
| 9 | **证据留存** | [WORKFLOW §5.3](../../WORKFLOW.md) + [证据 README](../engineering/evidence/README.md) 规定七字段；抽查实测：base / head SHA **多数有**（head 常只写"本提交"）· Blender 版本 **齐全** · **生成参数指纹无一份** · 母版 `sha256` **只有 1 处且截断** | 证据目录 + CI 的 `docs-integrity` + 四个校验器（死链 / 隔离 / 参数 / issue 契约） | 指纹与参数两格空着 ⇒ 门禁要求：状态声明**必须**带 `commit + Blender 版本 + 源资产 sha256 + 生成参数`（命令与参数），否则该格**按未记录处理**（[WORKFLOW §2.3](../../WORKFLOW.md)） |
| 10 | **Blender→Unity 准入分层** | 六层现在处于**不同深度**（见准入阶梯"现状"列）：目视接受 6 件 · 机械验收 6 件 · 导出 **1 件入库 + 1 件跑过未入库** · Unity 导入 **1 件（探针）** · Play 验收 **只有隔离 lab** · 运行时接入 **0** | 每层各有既有门禁（回显卡 / `E0–E6` / A4 / PlayMode 断言 / controller 与词条） | 把"Blender 里做出来了"说成"游戏能用"⇒ 门禁要求：**声明到哪一态，就只能主张哪一态的用法**；跨层主张（例：用 `BLENDER_VISUAL_ACCEPTED` 说"可玩"）视为**假完成** |

**泛化声明的前置**（与 §7.7 的接口路线解耦）

- 在**机器可读的 Motion Intent / 动作配方中间层**出现、**并由第二个不同动作验证**之前，"自然语言 → 游戏动画资产"只能主张到**受控样本**；触发条件仍留在[口径](动作描述口径.md) §1.1 的表里，本件**不授权**建造。
- #172 的接口路线（owner 选 A：维持现有桥、不接 MCP）**不自动提升**本文任何准入等级——facade 后面仍是同一个 addon，只搬调用面。

**本件不做**（照 #173 明确排除）

- 不修改、加固或替换活体桥；不实现 MCP Adapter / Motion Intent Schema / 取消机制 / 事务 / 独立验证器 / 载体 profile。
- 不导出或接入 `BendGripChairBack` 及其三条已知偏差；**不运行** Blender / Unity / 动作生成器 / 资产门禁。
- **不创建下游实施 issue**；未来的任一实施必须**逐项单独建 issue**、自带预期差分与回滚，**不得**把本节当批量实施授权。

**复核中登记的"与文档不符"五条**——owner 2026-09-16 裁：**只登记、不开 issue**；本件**不改 `code/tools/**`**

| # | 事实（实测） | 处置 |
|---|---|---|
| 1 | [危险点表](../engineering/危险点表.md) §四「多实例 lab 与母版 / 导出源」行引的 `:1861 / :3679` **已失效**（现分别是回显卡的"owner 的话"行与门边踏点判据行），且只引 **2 处**——实际有 **5 处**（`:1908 / :3729 / :4859 / :5635 / :6801`） | 候选队列：下次碰母版写回时补行号（按 [WORKFLOW §5.2](../../WORKFLOW.md)，行号只作展示、不参与判据） |
| 2 | `author_xbot_chair_grab.py:143` 的 `--force`（覆盖前确认这一类开关）**零消费**——全文件只此一行 | 候选队列（属上表风险 3 的缺口） |
| 3 | `data/hand_grip_cards.json` 的 `_meta.template` 固化 **Windows 绝对路径** `Z:\home\dog\game\...`（机器可读契约里带机器相关路径） | 候选队列 |
| 4 | [桥 README](../../code/tools/blender-bridge/README.md) §五 括注"都在危险点表 §七 有行"**不成立**（§7.7 已登记；本次复核按 12 个检索词实测 **0 命中**） | 维持 §7.7 的候选队列处置 |
| 5 | [危险点表](../engineering/危险点表.md) §四 同一行的**判据列**写的是「`git status` 看母版是否被动过」——**该判据对本仓母版不成立**：母版在 `.scratch/`、被 `.gitignore` 排除，`git status` **看不见**它被动过（可用判据 = 母版自检 V2 / V3 / V9 退出码 + `sha256`） | 候选队列（与第 1 条同批；本次已在风险 3 的规避列写明正确判据） |

**推翻路径**：若 C 日后被推翻（例如 owner 改判 A / B），本节标 **`SUPERSEDED`** 并恢复上一状态；任何加固或实施性变更由后续**独立 issue** 自带回滚方案（#173 §回滚）。

## 七·B、~~动作素材：`BendGripChairBack`~~ ⏸ **已撤回（2026-09-14）**

> **状态：撤回，不在仓库里。** 本次尝试的产出（动作 `BendGripChairBack`、探针 FBX、生成器
> `code/tools/author_xbot_chair_grab.py`）**已从仓库撤出**，仅存本页这段记录与 git 历史。
> 撤回**不表示**"做完了"或"做错了就删档"，而是：**需求描述口径未解决前不再产出成品**。

**为什么撤**：owner 目视三轮（站位侧 → 肘朝向 → 手指侧弯 → 手没放在椅背上）**均未通过**，
最后一轮 owner 判定"还是不对"。⇒ 双方卡在**"用自然语言描述姿势需求"这一层**，
不是实现层的能力问题（几何可达性、解析二骨解、手指屈曲轴自测都已跑通，读数可查）。
owner 将**另开 issue 专门解决语言描述口径**，之后再回来调动画。
> 🔧 **2026-09-14 已解决**：该口径已落为 **[动作描述口径.md](%E5%8A%A8%E4%BD%9C%E6%8F%8F%E8%BF%B0%E5%8F%A3%E5%BE%84.md)**（人机接口：三问骨架 · 回显卡 · 自由度表 · 参考系与轴 · 容差 ε · 验收判据，D1–D10 经 owner 逐题接受）。本动作的重做即该口径的**回归样本**（该文 §十 V-a…V-e）。

**撤回后仓库里留下什么**（都是**与这次动作解耦**的东西，继续有效）：

| 保留项 | 为什么留 |
|---|---|
| [Blender 侧错误分类](../engineering/blender%E9%94%99%E8%AF%AF%E5%88%86%E7%B1%BB.md) | **本轮最大的产出**：七类 12 处错误 + 开工/收工清单；owner 明确要求单开一页 |
| `export_xbot_action.py` 的 **bind pose 修复**（导出前回首帧） | 管线级缺陷，与具体动作无关；探针动作也受益 |
| `code/tools/blender-bridge/` · `measure_xbot_control_axes.py` | owner 点名要的工具 |
| 危险点表 §四/§七 的相关行 | 按位置索引的坑，与动作无关 |

**撤回的教训（供下次描述需求用）**——「自然语言 → 姿势」这一步，本轮有 5 处**判据错**与
2 处**污染**（详见错误分类页 A1–A5 / B1–B2）。共同点：**描述里没有可判据的几何量**，
于是我只能自己发明一个目标函数，而**发明的目标函数恰好指向了错的姿势**（例如
"肘向外+向后之和最大"鼓励肘外张、只用"指尖朝掌心最大"选到侧弯）。⇒ 若需求能直接给出
**可判据量**（如"肘相对肩-腕弦向后偏移 ≥ 60 mm、体侧 ≤ 40 mm"、"四指共用一条屈曲轴且指尖落在
顶杆远侧下方"），这类返工可以省掉。
