...
| ~~**主干 `check_clean_checkout` 常红**（2026-09-13 演练中发现）~~ → **✅ 2026-09-13 已修** | `code/tools/build_brain_skill_tree_windows.py:31` 的 docstring 里留着一条开发机路径（`C:/Users/9527/temp_build_brain_skill_tree.py`，历史注记）→ 该检查器 exit 1，**主干 CI 的 `docs-integrity` job 死在「干净检出可解析性检查」这一步**（run `34760300460` 实测；本机同款复现）。修法是改写那一行历史注记：**保留事实**（修复此前只存在于一份未入库的 Windows 临时脚本）**而去掉机器路径**——`.py` 里出现机器绝对路径正是该检查器要判的事，修复后 `check_clean_checkout` exit 0。**为何由 #128 顺手修**：它让 #128 声明的 `docs-integrity` 门禁无法真正转绿；owner 2026-09-13 裁定即刻修（#128 的"预期不变：任何源码"据此破例，改动仅为注释）。**修复后 CI 实测**：run `34760885707`（`4167ae7`）四个 job 全绿——此前连续 5 次 run 都是 `failure`（`docs-integrity` 死在同一步） |
