...
**第五处（最值得记的一条）**：初稿把 5 条就绪项判成"互不相干"，被 §4.3 的复核推翻——**语义独立不等于工作面独立**。由此新增规则 **I13**（并行就绪的多条 issue，其「预期差分」文件集合不得相交），并把「预期差分」从"人工判断内容"提升为**必须逐条写出文件路径**（否则集合无从计算）。这是"先记下被排除/被推翻的判断"这条纪律的收益：如果不复核，这个假并行会一直留在示范文档里当作正确做法被后来者照抄。
