...
| 校验器依赖 | `PyYAML==6.0.3` · `numpy==2.2.6` | [`code/tools/requirements.txt`](../../code/tools/requirements.txt) | 17 个生产校验器里**只有两个**需要第三方库：`validate_disease.py`（YAML frontmatter）与 `validate_ceiling_generator.py`（numpy——它要真跑一次生成器，#134）；NPC fixture 套件仅用标准库 |
...
NPC 素材生产校验器另有一个独立 mutation fixture 套件；它不计入上述 17 个生产校验器：
...
> 2026-09-12 修：编排器此前把校验器目录写成 `ROOT / "tools"`（Phase 3 之后该目录已不存在），于是 `get_active_validators()` 返回空列表——**跑了 0 个校验器却退出码 0**，是静默全绿。现已改为 `code/tools/`，注册表与 CI 的 17 个生产校验器逐项对齐，并加「找不到校验器即退出 2」的断言；#174 的 NPC fixture 套件也由同一注册表编排，但在 CI 中保持独立步骤。判据：编排器读数必须与 §三 的 `docs-integrity` job 一致。
...
| job | 覆盖 | 本机可复现 |
|---|---|---|
| `docs-integrity` | 17 个生产校验器（与 §2.1 同一循环）+ 1 个 NPC fixture 套件 + fixture 索引契约 | ✅ |
...
| 判据 | 要求 |
|---|---|
| 生产校验器与 NPC fixture | 17/17 生产校验器退出码 0；`test_validate_npc_materials.py` 独立退出码 0 |
