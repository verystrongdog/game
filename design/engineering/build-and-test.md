...
| 校验器依赖 | `PyYAML==6.0.3` · `numpy==2.2.6` | [`code/tools/requirements.txt`](../../code/tools/requirements.txt) | 14 个校验器里**只有两个**需要第三方库：`validate_disease.py`（YAML frontmatter）与 `validate_ceiling_generator.py`（numpy——它要真跑一次生成器，#134） |
...
| job | 覆盖 | 本机可复现 |
|---|---|---|
| `docs-integrity` | 14 个校验器（与 §2.1 同一循环）+ fixture 索引契约 | ✅ |
