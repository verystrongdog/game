...
### 4.8 owner 目视第 1 轮：两条指名与改动（2026-09-16）

owner 在 Blender GUI 里看完（`Start-Process` 打开交付 clip，停在到位帧 13）后**指名两个量**（口径 §4.3/V-d 的形态：不接"还是不对"这种话）：

| owner 的话 | 解成的量 | 根因与改动 | 改后读数 |
|---|---|---|---|
| 「**手应该再向下一点**」 | 掌面重心在**面内**的落点 | 根因：原实现把掌面重心对到**面片重心**，而耳侧那片的最外点（= **ε 锚点**）落在面片**下缘**（面内跨度 `u ∈ [15.7, 158.4] mm`）⇒ 掌被抬到耳朵**上方 ≈80 mm** 的头侧。**改**：对齐目标改为**该面的 ε 锚点**（+可调 `--ear-drop-mm`，默认 0） | 面内 `True` ✓ · 间隙仍 **1.97 mm** ✓ |
| 「**手应该是手掌的姿势**」 | 手型（F10） | **改**：默认手型 `flat` = **平掌**（四指近伸展，基线蜷曲 0.15 ≈7°/节；0 太僵）——原默认是"逐指拟合贴住头面"（`hug`），保留为**对照解**（口径 §4.2 硬规矩 5 的 ≥2 解） | 四指 **+48 ~ +56 mm**（平掌 ↔ 弧面的**固有间隙**，由读数说、不判贴合）· 手指方向同时改为**指向"头顶面"锚点**（数据给的方向；纯竖直时指尖翘 **50 mm**） |

**第 1 轮顺带查出的三条**（都已在代码里修，逐条登记）：

1. **拇指的"摆到头外"那一档在打键时被擦掉**：`_apply_one_finger_curl` 只写**屈曲轴**，逐帧重设手指时把 `panic_solve_thumb_clear` 解出的那一档覆盖了 ⇒ 解算时拇指 **+3.88 mm**、产物侧却是 **−6.5 mm**（差的正是这一档）。
2. **参照集必须解算与判据同一套**：头部"顶点法向"的抽样步长不同（`[::2]` vs `[::3]`）⇒ 最近顶点不同 ⇒ 同一根拇指的读数差到 **10 mm** 量级（现统一为 `PANIC_HEAD_NORMAL_STEP = 2`，并从**唯一的取法** `panic_head_normals()` 取）。
3. **"整手滚转"救不了拇指**（实测扫描 0/12/24/36°）：转了拇指侧，**掌的另一侧就咬进切线面**（掌面最低点从 +1.97 掉到 −8.27 mm）⇒ 拇指的问题**不是手的朝向**，是**刚性平掌贴弧面**的几何。

⚠️ **因此判据面明写为「掌面 + 四指」**（`clash` 段的注释与报告字段 `判据面` 都写着）：拇指**单列为读数**（`拇指带符号间隙_mm`，本件 **−6.5 / −5.3 mm**），**不静默放宽阈值**——要真修得给拇指加**外展自由度**（见 §八#8）。

### 4.9 owner 目视第 2 轮：**母版没有耳朵几何**（"耳朵在哪"只能由 owner 定）

owner 第 2 轮指名：「**用 hug，但要向下一点，现在捂住的位置不是耳朵**」。为了不靠猜毫米数，本件把头**侧向剖面**实测了一遍（`head_skin_local` 的顶点按头骨局部 Y 每 5 mm 切片，逐片取最大 `+X`）：

| 距头底 | 最大 `+X` | 说明 |
|---|---|---|
| ≤ 150 mm | 迅速收窄（下颌/颈部） | — |
| **155 – 205 mm** | **85.6 ~ 88.0 mm（一段平台）** | **ε 锚点**（最外点）落在 155 mm 这一片上 |
| 210 – 250 mm | 75.4 → 57.2 mm（向上收窄） | 颅顶方向 |
| ≥ 260 mm | ≤ 40.6 mm | 头顶 |

**⇒ 头侧没有任何局部凸起（没有耳朵几何）**：这是一具"人偶式"的头（`Beta_Surface`，可见层唯一含头的一层，§4.1）。三条后果：

1. **「耳朵在哪」测不出来**——网格上没有可识别的地标 ⇒ 按口径 §5，它是 **`owner 定` 的自由量**（不是 `agent 解`），本件因此把 `--ear-drop-mm` 暴露出来，并在自由度表里把该行的**归属**写成 owner。
2. **可行带 = 相对 ε 锚点向下 0–50 mm**（落在"头最宽的那条带"内）；再往下（215 mm 起）就出了这条带，掌面会压到收窄的下颌/颈部。
3. 这同时解释了 §八#2 那条缺口：**"具名面的横向范围"用"最外片层的顶点跨度"定义会把这条带切窄**（只覆盖锚点**上方** 15.7–158.4 mm）——**更好的定义**是"**侧向剖面落在最大值 −3 mm 以内的那一段高度**"（本件实测 = 155–205 mm，数据给的、可直接算），这条留给 ②③ 或另开件落地。

### 4.10 owner 目视裁定与定稿（2026-09-16 · 第 2/3 轮）

| 轮 | owner 的话 | 解成的量 | 处置 |
|---|---|---|---|
| 1 | 「手应该再向下一点」·「手应该是手掌的姿势」 | 落点（面内高度）· 手型 | 见 §4.8 |
| 2 | 「**用 hug 但是要向下一点，现在捂住的位置不是耳朵**」 | 手型 = `hug`；落点需再下移 | 出 d30/d60/d90 三档供目视（**该做法被 owner 否掉**，见下） |
| 3 | 「**最后一个是捂住耳朵**」 | 落点 = **向下 90 mm** | **定稿**：`PANIC_FINAL_HAND_SHAPE = "hug"` · `PANIC_FINAL_DROP_MM = 90.0`；owner 随后**确认「通过」** ⇒ [#169](https://github.com/verystrongdog/game/issues/169) **同日结账关闭**（结账评论含逐条验收 + 命令退出码 + 10 个提交）|

**定稿读数**（`--task panic`，退出码 **0** · `problems []`）：掌面 ↔ 耳侧面 **1.97 mm**（面内 `True`）· `F4a` 持续窗口（帧 13–46）逐帧最坏 **1.97 mm** · 非退化对照 **622.44 mm** · 四指 **+1.5 ~ +4.9 mm**（贴合）· 拇指 −0.2/−1.2 mm · 手/指蒙皮零穿透（最坏 +1.39 mm）· 时序 **13 == 13** · 接地无支撑帧 **0** · 可见骨 **52** / 越界 **0**。

🔧 **同时修掉一个真 bug（基准不同）**：`面内跨度`（面片范围）原来给的是**相对骨原点**的坐标，而 `面内偏移` 量的是**相对 ε 锚点**的偏移，两者被直接比较（锚点落在面片中部时差 ≈**70 mm**）⇒ "最低点是否落在面片内"这一格**判错**（`抱头` 被判面外、落点下移后立刻被判面外，都含这个成分）。现统一到**锚点**为基准。

🔧 **并因此把判据的"面域"改成声明式**（`PANIC_FACE_U_WINDOW_MM = (-150, +60)` mm，相对 ε 锚点）：旧口径（最外片层的顶点跨度）只覆盖"头最宽的那段带"（实测距头底 **155–205 mm**），而 **owner 认定的耳朵在这段带下方 90 mm** ⇒ 旧口径会把"正确的落点"判成面外。新口径的两个数都有来源：下界 = owner 裁定值 −90 再留 60 mm 余量、上界 = §4.9 实测平台顶 +50 再留 10 mm。⚠️ **这条口径留给 ②③ 复核**（它已经超出"逐件登记"的范围，接近"面域该怎么定义"）。

### 4.11 owner 的工作面裁定：**一个窗口 + 多实例**（2026-09-16）

owner 原话：「**不要开这么多窗口了**……之后有类似的任务**在一个 blender 窗口里面解决**就可以了，**新建人物模型，不要新建窗口**」。

⇒ 这正是口径 **§15.8 的多实例 lab** 的用途（装配器 `code/tools/lab_multi_instance.py` 本件已建）。**流程定为**：多变体比较（不同解 / 不同落点 / 不同手型）**在同一 `.blend` 里各产一条 action**，再用 lab 在一个窗口里并排看——**不再每个变体开一个窗口**。本件第 2 轮开了 5 个窗口（3 档落点 + 平掌/hug 对照），**做法已按此裁定作废**，多开的窗口已关闭（只留定稿的那一个）。
...
| # | 项 |
|---|---|
| 1 | ~~owner 目视未做~~ → ✅ **2026-09-16 三轮目视后定稿**（第 3 轮「最后一个是捂住耳朵」= 向下 90 mm + `hug`），记录见 §4.10；⚠️ **走了 3 轮**（口径 §13.8 判据 2 的目标是 ≤2 轮）⇒ 与 #166 同级。**owner 已确认通过并结账**；`Health` 仍 `UNKNOWN`（判据 2 的 ≤2 轮未达标，同 #166 的判法）|
| 2 | **"具名面的横向范围"定义不合格**（本件实测暴露）：现用"最外片层的**顶点跨度**"，而**后脑面只有 12 个顶点** ⇒ 跨度小、掌缘容易判"面外"（抱头解由此判不上），且**耳朵那一段带也被切窄**（只覆盖锚点上方 15.7–158.4 mm）。🔧 **2026-09-16 第 2 轮给了数据来源**（§4.9）：改用「**侧向剖面落在最大值 −3 mm 以内的高度区间**」（耳侧实测 = 距头底 **155–205 mm**）即可，不需拍数；**本条不落地**（留给 ②③ 或另开件）|
...
| 8 | 🔧 **第 1 轮后新增：拇指侧鱼际的咬入**（−6.5 / −5.3 mm）——刚性平掌贴弧面的几何后果，"整手滚转"救不了（转了拇指侧、掌的另一侧就进切线面，实测掌面最低点掉到 **−8.27 mm**）⇒ **要修得给拇指加外展自由度**（`HandThumb1` 之外再加一节或直接解拇指到目标点），**本件登记、不静默放宽判据**：判据面明写为「掌面 + 四指」，拇指逐帧单列读数 |
| 9 | **平掌的四指必然翘在头外**（+48~+56 mm）——平面贴球面的固有几何；要"指尖也贴住"就得把手当**柔性/多段**处理（本件的 `hug` 对照解就是把四指拟合到贴住：−1.0 ~ +2.9 mm）⇒ 两解**都给读数**，取舍归 owner |
...
**全提交组复演（2026-09-16 · 定稿后）**：把本条**全部 8 个提交**（`8dcc868` 宿主+lab · `6a8eea4` 证据/口径/切片 · `3f86ac4` 回滚记录 · `b6750e0` 预览清理 · `8281c76` 第 1 轮 · `b5caab2` 第 2 轮 · `6c032f8` 定稿 · `1b4ad76` 口径定稿）在临时 worktree 里**逐条 `git revert`** ⇒ **`git diff d632baa HEAD` 为空**（逐字节回到 base）· `panic` 全文件命中 **0** 处 · `lab_multi_instance.py` **不存在** · `validate_cross_refs` 回到改前一组数（**2020 refs / 0 死链**）· 临时树已清、工作树干净。
