...
| 未做的事 | 未做 Blender **5.1.2 横向对照**；未做**完整 Editor 重启**后的复现；**"逐帧目视"的目视部分未由人完成**（只到"两端截图已产出 + 三项数值代理"，见 §六·4）；危险点表未登记本条的三条坑（不在本条「预期差分」内，进候选队列） |
...
4. **"逐帧目视"的人眼部分未完成**：执行者（agent）无图像输入能力，只产出"两端截图 + 三项数值代理"。**截图未经人眼复核**——这一条需要 owner 或有视觉能力的执行者补看（截图在 `.scratch/shots/`，为过程物、不入库）。
