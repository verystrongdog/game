...
**结论：可干净回滚**——**`f4152f6..HEAD` 范围内的全部提交**（正典改动 6 条 + 证据文件/索引/引用口径/补漏 3 条，共 9 条）逆序 revert 后，工作树与 base `f4152f6` **逐字节相同**（`git diff` 退出码 0）。本阶段全部为设计面文本改动，无数据迁移、无删除、无玩家行为，故回滚不需要额外方案。演练后 `git worktree remove --force` + `prune`，主工作树未受影响（`git status --short` 无输出）。

> 绑定：本证据的**内容**对应 head `32770e4`（8 文件落地的最后一个改动提交）；其后的 `c75c866` / `0945988` / `a770215` **只改本文件、证据索引，以及 §六 登记的三处补漏**（`任务目标系统 §九` 的门控承诺措辞 · `游戏循环 §2.2` 的「恢复量不因动机而变」一句），不改任何结论。
