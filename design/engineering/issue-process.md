...
| 情形 | 处置 |
|---|---|
| 判据 2 的缺口是**"这条该归哪一批 / 哪一节"**这类**编排问题** | 写进该 issue 的「**依赖**」或「**明确排除**」一节（如实登记为在案缺口），**由 triage 判一次**。**不得**把它前置成"先立一节设计文档再建 issue"——那正是 §1.1 拦的形态 |
| 判据 2 的缺口是**机制本身没有设计面**（要做的东西在 `design/` 里没有对应权威） | 这才是真回退：**先落决策，再导入**（[项目规约 §一](../conventions/README.md) 硬约束） |

**判据**：缺口能不能用**一句 `待建:` / 一条在案缺口**说清？能 ⇒ 编排问题，进正文、由 triage 判；不能 ⇒ 机制缺设计面，回讨论。

> **先例（2026-09-16 · [#180](https://github.com/verystrongdog/game/issues/180)）**：建 issue 时发现「本件归属哪一批未声明」（`动作描述口径.md` §15.7 的批次是**对角线 3 件**，其立论不容第四行）。当时**先**把缺口写进了 #180 的「依赖」一节（合规处置），**又**额外前置成"先新写 §十六 一节"（**多做的**）。owner 指出：「**怎么建一个做动作的 issue 这么多事**」。⇒ 本节由此而写：**先例 #169/#170/#171 的"先声明批次再建 issue"是那批的实情，不是通用前置**；`presentation/动作描述口径.md` §十六 是这次立的新批，**不是**以后每件都要走的步骤。
