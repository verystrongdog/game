...
| I6 | 声明的 gate id 存在于 `gates.json`；若该 gate **当前不可用**，本 issue 不得标 `state:ready-for-agent`。`门禁:` 中的脚本路径**必须真实存在**（无条件硬判，不走「交付物」豁免），且**须像脚本**（`.py` / `.sh` / `.bash` / `.ps1` / `.cmd`）；交付新门禁的 issue 按 [§4.1.1](#411-三个受控取值域) 例外 1 引用**已存在**的门禁验证自己 | 查表 + 文件存在性 | ✅ |
...
| I10 | 正文不得引用**垃圾桶路径**（原 `design/archive/trash/` 已于 2026-09-13 整份移出仓库、现本地为 `.trash/`——**两个位置都不得指向**）、不得使用 `term_registry.json` 中 `deprecated` 的术语 | 复用既有判据 | ✅ |
...
| I10 | 不做「⚠️ 已废弃」行豁免（比 `validate_trash_isolation.py` 严），故警告偏多；只覆盖 `term_registry.json` 里有替代说明的 deprecated 条目。**垃圾桶路径部分为正文子串判定**（不要求该路径存在），故两个位置（`design/archive/trash` / `.trash`）都被覆盖 |
| I12 | 计数的列表项接受 `- [ ]` / `-` / `*` / `+` / 有序列表 |
| I6 | 无法解析成 gate id 或命令的**散文化**门禁行给警告而非失败（否则"本机不可用，替代验证见…"会误报）。**已收紧（2026-09-13）**：`judge_gate_line` 抽到的路径**须像脚本**（`.py` / `.sh` / `.bash` / `.ps1` / `.cmd`）才判 `script`——此前一行散文里提到的**文档**路径会被当成被检查的脚本：实测 #134 的门禁行因括号内的解释文字提到了 `design/engineering/issue-process.md` 而**空过**（该文件存在 → PASS），**真正该检查的新门禁名从未被检查**；收紧后该行判 `unknown` → FAIL（#134 的门禁行已同步修正）。**仍靠人工的一条**：`门禁:` 每行只写 gate id 或命令、不写解释文字（解释放「动机」）——纯散文行只出警告不出失败，符合 [§1.1](#11-本文不得变成仪式) 判据 2「有具名消费时刻」。脚本路径的存在性是**无条件硬判**——本表此前曾把一条从未实现的例外写成 I6 的规则（"只要求「交付物」里明列"），2026-09-13 校正为文档向代码看齐 |
...
> **本次授权（2026-09-13，逐次确认的一次例外）**：owner 授权 agent 在 **#128** 上
> **附一条证据评论并置 `closed`**。范围仅限该 issue 的**一条评论 + 状态变更**——
> 不改正文、不改标签、不动其他 issue。据此关闭时，评论须含 base/head SHA、
