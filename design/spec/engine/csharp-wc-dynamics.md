...
连续方程（[运行时状态模型](../../rules/skill-tree/%E8%BF%90%E8%A1%8C%E6%97%B6%E7%8A%B6%E6%80%81%E6%A8%A1%E5%9E%8B.md) §4.1）：
...
- Δ = 回合时长 = 1.0 秒（[皮层动力学-通用层](../../rules/skill-tree/%E7%9A%AE%E5%B1%82%E5%8A%A8%E5%8A%9B%E5%AD%A6-%E9%80%9A%E7%94%A8%E5%B1%82.md) §4.3「回合时长（默认 1.0 秒）」）
- σ(x) = 1 / (1 + exp(−(x − 0.5)))——即 [皮层动力学-通用层 §4.2](../../rules/skill-tree/%E7%9A%AE%E5%B1%82%E5%8A%A8%E5%8A%9B%E5%AD%A6-%E9%80%9A%E7%94%A8%E5%B1%82.md) 的 v=1.0、θ=0.5
- **同步更新**：所有 h_j 由 t 时刻（上一回合末）的 a 计算完毕后，再统一写出全部 a_j(t+Δ)——[运行时状态模型 §7.1](../../rules/skill-tree/%E8%BF%90%E8%A1%8C%E6%97%B6%E7%8A%B6%E6%80%81%E6%A8%A1%E5%9E%8B.md) 步骤 3「W·a 使用上一回合末的 a 值（同步更新避免顺序偏倚）」
...
| B2 | 静息吸引子 ≈0.5 非 0.10 | [回合战斗流程](../../rules/%E5%9B%9E%E5%90%88%E6%88%98%E6%96%97%E6%B5%81%E7%A8%8B.md) §3.3「未显著激活回到静息基线 0.10」 | a(t) 实测吸引子 ≈0.5（0.10 仅初始值，运行时状态模型 §7.3） | csharp-engine plan §十二-5：审计数值证明 0.10 非不动点；b=0 实测 48 活跃节点吸引子 ∈ [0.4986, 0.49951]（max=0.4995055，2026-08-13 python + 审计三专家独立复算） |
| B3 | 解析解推论：动力学实质 = 固定点迭代 | [皮层动力学-通用层 §4.3](../../rules/skill-tree/%E7%9A%AE%E5%B1%82%E5%8A%A8%E5%8A%9B%E5%AD%A6-%E9%80%9A%E7%94%A8%E5%B1%82.md)「slow 节点跨回合平滑过渡，保留历史」 | 全节点单回合收敛 ≥99.87%（slow e^(−6.67)=1.27e-3 残留 0.127%）→ 每回合 a ← σ(W·a+b+s) 近乎无记忆 | 2026-08-13 任务issue Q2 裁决：实现照解析解，文档叙述修正并入 plan §十三-8 Euler 一致性清扫（§4.3 已在清单内），本 spec 只声明不修订。顺带修正项：皮层动力学 §4.2「σ(0)≈0.006」实测应为 0.377541——一并列入清扫清单 |
...
| AC-7 | bimodal s=2.0（合成 W=0、τ=0.01、s=2.0、b=0）→ a' == σ(2.0)；输出 ∈ [0,1]、无 NaN/溢出。**期望值用 MathF 自算**（同 AC-6/AC-9 模式），不写字面量——float32 计算值（MathF 实测）0.81757444 与字面量 np.float32(0.817574476)=0.81757450 差 1 ulp | 0.817574476 |
...
| AC-12 | 极端输入无 NaN/上溢（真实 W，a 全 1.0、b 全 2.0、s 全 2.0）→ 输出 69 维全部有限 ∈ [0,1] | h ≤ 行和+4 ≤ 5 → σ(5)=0.989013；实测真实输出 max=0.988996（Precuneus，τ=slow，含残留项——f64/f32 双路径一致）∈ [0.9707, 0.9890] |
...
- [ ] **方向契约明确**：行=接收者（C2），AC-9 哨兵锚点锁定（Pericalcarine→Amygdala 唯一非互惠对——fid 名大写，见 AC-9 大小写注）
...
| v1.1 | 2026-08-13 | 修正：E1 B3「≥99.9%」→「≥99.87%」；E2 AC-9 名字 Pericalcarine 大小写 + 分句说明；E3 B2/AC-5 上界 0.4995→0.49951；W1 任务issue bimodal 名单修正（5/12 错）；W2 e^(−100) double/float32 双值标注；W3 AC-12 σ(5)=0.989013；ℹ️ 结转精确化、不覆盖补 step 7/9、C1 RowFids 不校验、AC-7 MathF 自算注、§三 NaN 行为声明、§四 Δ 共享漂移注。🔧 Δ审计后 L1 清扫（免重审）：AC-12 max 0.988984→0.988996（含 slow 残留项）、AC-7 加 MathF 限定、§七 Pericalcarine 大写、任务issue D6/行37 同源残留、map.md 状态行 | 全量审计退回（3❌） | Δ审计 |

---
*创建: 2026-08-13 | 更新: 2026-08-13 | 版本: v1.1*
*关联: [csharp-engine plan](csharp-engine-roadmap.md), [运行时状态模型](../../rules/skill-tree/%E8%BF%90%E8%A1%8C%E6%97%B6%E7%8A%B6%E6%80%81%E6%A8%A1%E5%9E%8B.md), [皮层动力学-通用层](../../rules/skill-tree/%E7%9A%AE%E5%B1%82%E5%8A%A8%E5%8A%9B%E5%AD%A6-%E9%80%9A%E7%94%A8%E5%B1%82.md), [任务issue 01](../../archive/grilling/csharp-wc-dynamics/design/issues/01-wc-dynamics-spec.md)*
