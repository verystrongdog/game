...
> Layer 3（CSTC Gurney 门控）引擎规格——`CstcGating` 把 WC 激活 a(t) 与 DA tone 转化为 15 个 Gurney 群体激活 + 3 个 gate + 3 环路 salience（每回合 Phase 3 的最后一个数值引擎）。版本: v1.1
...
| 不覆盖 | SpeedScoreCalculator/TurnOrderBuilder（step 7）、Phase 4 gate_bonus 结算（应用侧——[运行时状态模型](../../rules/skill-tree/%E8%BF%90%E8%A1%8C%E6%97%B6%E7%8A%B6%E6%80%81%E6%A8%A1%E5%9E%8B.md) §6.5 公式由 step 8/10 消费）、Phase 编排与事件（step 9-11）、NPC salience 竞争（step 12） |
...
本规格的决策依据 = [任务issue 01](../../archive/grilling/csharp-cstc/design/issues/01-cstc-spec.md) D1-D13 + Q1/Q2/Q3 裁决（其「数据实测」表为全部计数与锚点出处，2026-08-13 python 实测）。本 spec 的 Step 锚点另经第二轮实测复算（gate 时刻语义修正——见 B1），锚点数值以本 spec §六 为准。
...
| B4 | **构造函数 `CstcGating(GameData)` 为 spec 新增**——plan §4.4 仅给 Step 签名；CI 行索引需构造期解析，故增构造入口（语义见 §三） | 任务issue 01 范围行已单列构造（仅 Step 标注 plan §4.4 签名）；先例：csharp-tone spec B1（Step 签名加参同样入偏差表） |
...
### 2.3 CI 行索引（构造期解析产物；公开只读访问器 CiRows——§三）
...
新文件 `code/src/YouAreNotTheFish.Core/Engine/CstcGating.cs`（Engine/ 目录已存在；plan §3.2 L2 约定：纯函数、无副作用、无 RNG）。
...
### 4.3 DA 混合权重（按环路枚举常量名显式配对）

| 环路（CstcLoop 枚举常量名） | wVta | wSnc = 1 − wVta |
|------------------------|------|-----------------|
| Somatic | 0.2f | 0.8f |
| Cognitive | 0.4f | 0.6f |
| Limbic | 0.8f | 0.2f |

配对方式：实现按枚举常量名 switch/if 显式映射（Somatic→0.2 / Cognitive→0.4 / Limbic→0.8）。**禁止按 CstcLoop 枚举序数建表**（枚举序 None=0/Somatic=1/Limbic=2/Cognitive=3/Global=4 与上表行序不同——按序数配表 Limbic/Cognitive 互换）；亦不按 §6.3 文档列表序（limbic→cognitive→somatic）。与约束 C6、LoopSalience 已交付文档警告一致。
...
### 4.5 ramp 输出函数（每群体 O = ramp(a, e, m)；运行时状态模型 §6.3 公式内联）
...
| # | 约束 | 违反处置 | 来源 |
|---|------|---------|------|
| C1 | CI fid→行号解析镜像 WMatrixBuilder Step 1（canonical = Wsensory.RegionIds，fidToRow 字典）；CI fid 未命中 RegionIds → KeyNotFoundException（隐式，同 WMatrixBuilder 姿态不防御——数据契约由数据层双射锁定） | — | csharp-wmatrix spec Step 1 行序契约 |
...
**迭代语义（AC-4/9/10 统一）**：所有「迭代 → gate 锚点」断言**固定 300 回合**，不使用 max|Δ| 终止判据。依据（2026-08-13 实测）：STN↔GPe 互耦跨回合收缩率 = √0.9 ≈ 0.9487/回合（环增益 W_STN_GPe×W_GPe_STN = 0.9；e^(−25) 只作用于单步内瞬态）——固定点收敛实测需 251（c=0）/ 253（c=1, DA=0）/ 298（c=1, DA=1，f64）回合；float32 下 1e-7 判据轨道依赖（AC-9 情形 a≈0.3 处极限环 max|Δ| ≈ 3.9e-7 永不达标），不可作为终止条件。300 回合实测 gate = 0.8434211 / 0.8065790 / 1.0，均在 1e-5 内。

### 构造与成员

| # | 验收标准 | 锚点/断言 |
|---|---------|----------|
| AC-1 | CI 成员资格（fid 级，Q3=B） | CiRows[CstcLoop.Somatic/Cognitive/Limbic] 经 Wsensory.RegionIds[row] 反查得 fid 集 == §二 2.3 表名单（SetEquals）+ 计数 3/7/12；CiRows 键集 == {Somatic, Cognitive, Limbic}（不含 Global/None）；global 0 成员不报错 |
...
| # | 验收标准 | 锚点/断言 |
|---|---------|----------|
| AC-4 | c=0 门控抑制（plan §十「c_loop=0 门控抑制」） | synthetic a = 全零（c=0 三环路）→ Step(prev=零)：a1 = [0, 0, −0.2, 0.225, 0.165]，gate1 = 0.635（三环路相同——c=0 时 DA 不参与 u_SD1/u_SD2）；迭代 Step 固定 300 回合 → gate = 0.843421（1e-5；300 回合实测 0.8434211，max\|Δ\|<1e-7 首达 251 回合——√0.9 收敛率见 §六 迭代语义）且 **gate < 1**（门控抑制成立，对比静息 gate=1.0） |
| AC-5 | 静息 gate = 1.0（整合，M1 链） | 测试重建 M1 静息 a（WMatrixBuilder.Build + CorticalBias.Compute(baseline) + WcDynamics.Step ×30；链参数同 csharp-tone AC-15：s=0 零向量、baseline tone、a(0)=0.10）→ Step(prev=零)：三环路 gate1 = 0.635；Step(prev=第一步结果)：三环路 gate2 = 1.0（1e-5） |
| AC-6 | 静息 c/DA 锚点（行序正确性锁定） | 同 AC-5 链：LoopSalience C = 0.665469/0.696963/0.692102（somatic/cognitive/limbic，1e-5；锚点源 = csharp-tone 工作issue 01 附录 M1 实测值表）；Da = 0.48/0.46/0.42（baseline tone Ne=0.3/VTA=0.4/SNc=0.5/Ht5=0.5，1e-5） |
...
| AC-9 | DA=1 → m_SD2=0（plan §十「DA 极端 0/1 下 ramp 斜率」+ 偏差 B2 除零防护） | tone(VTA=1, SNc=1)（DA=1 三环路）、synthetic a 全 1.0（c=1）、prev a_SD2=1.0：a1 = [2.0, 0.0, 0.8, 0.225, 0.165]（SD1 饱和 2.0；SD2 = e^(−25) ≈ 1.39e-11 ≈ 0——**非逐位零**，按 1e-5 容差断言，勿与 AC-11 逐位模式混用；m=0 除零防护生效不抛异常）；O1 = [1.0, 0.0, 1.0, 0.425, 0.365]（m_SD1=2 饱和点 a≥0.7 → O=1.0；O_SD2 ≡ 0）；gate1 = 0.635；迭代 Step 固定 300 回合 → gate = 1.0（1e-5；第 2 回合即达——a2_GPi = −0.2275 < e_GPi = −0.2 → O_GPi=0，之后恒真） |
| AC-10 | DA=0 → 对称斜率（m_SD1=m_SD2=1） | tone(VTA=0, SNc=0)、synthetic a 全 1.0（c=1）、prev 零：a1 = [1.0, 1.0, 0.8, 0.225, 0.165]，O1 = [0.8, 0.8, 1.0, 0.425, 0.365]，gate1 = 0.635；迭代 Step 固定 300 回合 → gate = 0.806579（1e-5；300 回合实测 0.8065790，max\|Δ\|<1e-7 首达 253 回合（f64）/ 237（f32））。注：DA=0 时 gate 对 c 非单调（极小 0.7771 在 c=0.2 处），0.806579 是 c=1（c 域端点）处的固定点值、非区间极值 |
...
| AC-12 | 无 clamp 负值（D11/Q2=A） | AC-11 第二步 a_GPi = −0.4575 原样返回（不抬升到 0——**唯一可证伪 clamp[0,1] 违规的断言**）；网格扫描 c∈{0, 0.3, 0.5, 1} × DA∈{0, 0.5, 1} 各迭代 20 回合 → 全部 a ∈ [−1.5, 2.0]（§三 值域），负值合法。注：网格断言为**包含性断言**——验证输出落在值域内，不验证 clamp 有无（e^(−25)≈0 使输出对输入历史不敏感，clamp[−1.5,2.0] 在当前参数下不可观测） |
...
1. 数学公式逐项对照——u 五式/gate/DA 混合与 [运行时状态模型](../../rules/skill-tree/%E8%BF%90%E8%A1%8C%E6%97%B6%E7%8A%B6%E6%80%81%E6%A8%A1%E5%9E%8B.md) §6.3/§6.4 及 plan §4.4 逐字一致（含 W_SEL_GPe=0.0 项保留、per-population e 表、无 clamp）；ramp 公式内联见 §四 4.5（含 B2 m≤0 分支）
...
7. 偏差声明完整——B1（gate 时刻）/B2（m=0 防护）/B3（残差上界）/B4（构造入口新增）+ Q1/Q2/Q3 裁决标注，实现注释逐条对应
...
| v1.1 | 2026-08-13 | 审计修复（1❌+5⚠️+7ℹ️+1 REFUTED 残留内核，详见 audit/report.md）：迭代语义统一固定 300 回合（AC-4/9/10 + §六声明）；CiRows 公开访问器（AC-1 可测）；ramp 内联 §4.5；§4.3 配对措辞防 ordinal 陷阱；AC-12 包含性断言声明；B4 构造偏差入表；C1 KeyNotFound 文档化；AC-9 SD2 ≈0 注 / AC-10 端点与非单调注；AC-5/6 链参数补全；任务issue D2 依据改写 + #3 行标注 | 全量审计退回（E1 必须修复） | Δ审计（变更章节 + 半径扩张） |
| v1.1（🔧 修正） | 2026-08-13 | L1 笔误：变更日志 v1.1 行审计计数「1❌+6⚠️+6ℹ️」与 audit/report.md 缺口汇总（1❌+5⚠️+7ℹ️+1 REFUTED）不一致，计数改正 + 补漏任务issue D2/#3 bullet | Δ审计 I-1（ℹ️，CONFIRMED） | 免重审（L1 笔误） |

---
*创建: 2026-08-13 | 更新: 2026-08-13 | 版本: v1.1*
*关联: [csharp-engine plan](csharp-engine-roadmap.md) §4.4/§十/§十三-3, [任务issue 01](../../archive/grilling/csharp-cstc/design/issues/01-cstc-spec.md), [运行时状态模型](../../rules/skill-tree/%E8%BF%90%E8%A1%8C%E6%97%B6%E7%8A%B6%E6%80%81%E6%A8%A1%E5%9E%8B.md) §6/§三, [csharp-tone spec](csharp-tone.md)（M1 链 + AC-15）, [csharp-wc-dynamics spec](csharp-wc-dynamics.md) §四（Δ 先例）*
