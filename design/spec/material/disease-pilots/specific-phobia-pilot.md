...
> 规则同 [ptsd-pilot.md](ptsd-pilot.md) §6.6：tone 从脑干广播边群推导，bias 从 CSTC 环路边推导；量级沿用 NPC AI §4.2 标签校准。正常人基线：NE 0.3 / DA_VTA 0.4 / DA_SNc 0.5 / 5HT 0.5。
...
**处置规则总结**：5 条旧链路全部直接映射保留（无语义转移、无废弃）——SP 旧链路端点对在三体图中全部存在；4 条 ⭐新增边全部命中现有边（「病理新增」从严，见 [ptsd-pilot.md](ptsd-pilot.md) §6.4）。
