...
### Q9（2026-08-27）B1 十类 paired 清单 ✅ 定案（v7 D8 落地确认）

- **决策 D8′（本批）**：十类 = 负 4（#1 随机/#2 纯前馈/#3 数字电路/#4 通信网络）+ 正 3（#5 生物神经网络/#6 人脑数据/#7 自模型控制器）+ 判别 3（#8 反馈控制器/#9 细胞自动机/#10 RL 智能体）；每类 representative + 预期负控制，paired 只改声明目标机制（来源：issue #94 Q9 + ChatGPT 分享 t_6a8f233da3588191bec560b5eaf9a05d；编号 D8′ 避免与 v7 §七 D8 冲突）。
...
### Q10（2026-08-27）无自然 M/A 系统的 M/A 实例化规则 ✅ 定案（v7 D10 落地确认）

- **决策 D10′（本批）**：①M/A 是 benchmark 的**规范输入**（非从系统推断的事实语义）；三类分治：负类 #1-#4 M=∅/A=∅（SM′=0 是规范选择+退化条款结果，不解释成「证明无自模型结构」）；正类 #5-#7 按构造指南翻译给规范 M/A；判别组 #8-#10 按预先声明的结构规范选 M/A（不声称自然语义）（来源：issue #94 Q10 + ChatGPT 分享 t_6a8f23e1005c81919b662d257de2d8d5；编号 D10′ 避免与 v7 §七 D10 冲突）。
...
| 决策 | 摘要 | 写入文件 | 位置 | 状态 |
|---|---|---|---|---|
| D1 | 完整 CS′_local(C) + 分层验收 | design/decisions/ + grilling-94.md | #94 条目「D1 范围」 | ✅ 已验证 |
...
| D5 | F8′ 构造（z1,z2） | 决策树 + 脚本 make_f8p_step | 决策树 D5 | ✅ 已验证 |
| D6 | XOR→直接依赖闭合后修正 | 决策树 + 脚本 D6 注释 + v7 衔接注记 | 决策树 D6 | ✅ 已验证 |
| D7 | 脚本 2 装载 + 断言对象=SM′ | 决策树 + rb_test.py | 决策树 D7 | ✅ 已验证 |
| v7 D8/D9/D10 | 十类 3/4/3 + 失败线 + M/A 规范输入 | 决策树 + benchmark.py | 决策树「v7 D8/D9/D10 落地确认」 | ✅ 已验证 |
| 实测结论 | 三脚本验收 | 决策树「实测结论」 | #94 条目 | ✅ 已验证 |

### 一致性清扫（Step 4 🔥）

- grep `y1^m1|XOR|直接依赖`：仅命中决策树新条目（D6 记录）——无活跃文档残留旧构造 ✅
- grep `F8′` 活跃 md：v7 §六（角色级描述，不需改）+ v6（历史锚点）+ 决策树 —— 无路径断裂 ✅
- 本批无废弃概念/废弃数字/垃圾桶移动 ✅
