...
| 归属 | [#116](https://github.com/verystrongdog/game/issues/116) 配额表驱动核验批次（平凡实例化裁定 D6）——[批次 2 总览（全量解锁）](%E6%80%BB%E8%A7%88.md) G4 行 |
...
| 父类 | 奖赏系统障碍（`design/entities/diseases/持续性抑郁障碍.md` frontmatter） |
...
| PDD 机制层设计 | 慢性持续型：无急性跳变、持续消耗；CGI-S 3-6；重档（双重抑郁叠加 CGI 6）临时启用 MDD 跳变表 [待 #88 定案] | `design/entities/diseases/持续性抑郁障碍.md` :48；persistent-depressive-pilot §五 |
...
> 跨锚差异材料（供裁决）：DSM-5 = "合并主义"（慢性 MDD ∪ Dysthymia = PDD，单类 + specifier 细分）；ICD-11 = "分离主义"（发作性抑郁与慢性心境恶劣并列三类）。项目疾病目录同时收录 MDD 与 PDD 两个独立条目（机制层各自建模，见 `design/entities/diseases/`），与**哪一锚**对齐决定"慢性 MDD 样本归 MDD 还是 PDD"的边界——**本文件不表态，只留两侧材料**（§三 反样本 1 / §四 材料 5）。
...
| A4 | 项目机制层已把 PDD 定为"慢性持续型（无急性跳变）"，与 MDD 的"急性阈值跳变"相区分——项目内对"PDD = 慢性"与 DSM 对齐 | `design/entities/diseases/持续性抑郁障碍.md` :48 | 项目内一致材料（非外部独立证据） |
...
| B4 | 项目侧：PDD 重档（CGI 6）= "双重抑郁叠加"并临时启用 MDD 跳变表——项目机制层已有"叠加"概念预留 [待 #88 定案] | `design/entities/diseases/持续性抑郁障碍.md` :48；persistent-depressive-pilot §五.2 | 项目内"叠加形态"先例（非资格材料，机制层） |
...
|--------|-------------------|------------------------|------|------------------------------|
| **MDD 发作-缓解型**（recurrent/episodic） | 低落、快感缺失、反刍——与 PDD 同谱系同标签（项目两病默认标签均为「反刍+过度抑制」） | MDD = **发作 ≥2 周**、发作-缓解时相结构，缓解期可完全无症状；PDD = **≥2 年多数日子低落、无 >2 个月无症状期**（A/C 条）；DSM-5 鉴别段以时程与缓解性区分 | DSM-5（APA 2013）MDD A 条 vs 300.4 A/C 条；`design/entities/diseases/重度抑郁症.md`（发作期建模） | 有 DEPRESSIVE_EPISODE_HISTORY（发作史声明）≠ 慢性模式；发作史原子不得充当 PDD 资格（§四 材料 4 反对面） |
...
| **环性心境障碍** | 长期心境不稳、抑郁样波动、低落时段（项目与 PDD 同属 丧失/哀悼 候选集） | 环性 = **亚阈值两相波动**（轻躁狂样 + 抑郁样时段），抑郁样时段不达 PDD 的持续低落语义；PDD **E 条显式排除"曾满足环性标准"**（两病互斥）；DSM-5 鉴别段：环性心境不稳 vs PDD 持续低落 [逐字 [需核验]] | DSM-5（APA 2013）300.4 E 条 + 环性标准；转化接口 :211 CYCLIC_MOOD_HISTORY；`design/entities/diseases/持续性心境障碍.md` | 项目 CYCLIC_MOOD_HISTORY（亚阈值波动病程声明）若 =1 → PDD 排除候选；"波动"≠"持续低落"是区分核心（PDD 原子语义须落在"持续/多数日子"而非"反复波动"——与 CYCLIC 镜像对称） |
...
| 人格障碍（BPD 慢性空虚/烦躁；分裂型等） | 慢性不快、空虚、无望感（与 PDD B 条第 4/6 项表面重叠） | BPD = 人格模式（人际/情绪/冲动/身份域），其烦躁为**应激反应性小时尺度**（批次 1 已核验）；PDD 无跨情境人格模式要件；DSM-5 鉴别：PDD 与人格障碍可共病、按主诉区分 | DSM-5（APA 2013）BPD Criterion 6 时程括号；[BPD 核验证据收集（批次 1）](../eligibility-batch1-sexual-trauma/BPD-%E6%A0%B8%E9%AA%8C-%E8%AF%81%E6%8D%AE%E6%94%B6%E9%9B%86.md) | 慢性不快不指向 PDD 唯一（跨诊断）；候选集层面 BPD 由事件类型门（性创伤/社会伤害/忽视遗弃）召回，与 丧失/哀悼 召回不重叠 [材料] |
...
- 本批次并行产出：批次 2 各族证据收集（SSD 已裁决；G1 进食族 / G2 焦虑谱 / G3 SZ 谱 / G5 ASPD / G6 拖延 并行运行中，见 [批次 2 总览](%E6%80%BB%E8%A7%88.md)）。

---

*创建: 2026-09-06 | 更新: 2026-09-06 | 状态: 证据收集完成（待用户裁决）*
*关联: [批次 2 总览](%E6%80%BB%E8%A7%88.md), [批次 1 总览](../eligibility-batch1-sexual-trauma/%E6%80%BB%E8%A7%88.md), [SSD 收尾裁决点](SSD-%E6%94%B6%E5%B0%BE-%E8%A3%81%E5%86%B3%E7%82%B9.md), [转化接口 §3.2.2/§3.2.3](../../../../rules/skill-tree/%E5%88%9B%E4%BC%A4%E8%AE%B0%E5%BF%86%E8%BD%AC%E5%8C%96%E6%8E%A5%E5%8F%A3.md), [持续性抑郁障碍](../../../../entities/diseases/%E6%8C%81%E7%BB%AD%E6%80%A7%E6%8A%91%E9%83%81%E9%9A%9C%E7%A2%8D.md), [重度抑郁症](../../../../entities/diseases/%E9%87%8D%E5%BA%A6%E6%8A%91%E9%83%81%E7%97%87.md), [持续性心境障碍（环性）](../../../../entities/diseases/%E6%8C%81%E7%BB%AD%E6%80%A7%E5%BF%83%E5%A2%83%E9%9A%9C%E7%A2%8D.md), [grilling-113 决策记录](../../grilling-113-disease-eligibility/grilling-113-disease-eligibility.md), [决策树 #113 Q2-11/Q2-12](../../../../decisions/README.md)*
