...
---
*创建: 2026-09-12 | 更新: 2026-09-14（🔧 四轴表补一行「Unity 动画约束与接触修正」（#157）——此前未登记致 I4 无法判定。该行先以 `Design: CANDIDATE` 登记（共同样本文档当时是草案 ⇒ I4 按规则判 FAIL，属正确信号），owner 同日接受文档后改 **`ACCEPTED`**，FAIL 消失。此前批次：2026-09-13 四轴表与 §三/§四 回填：`Unity 资产身份` 行 `NONE`/`UNKNOWN` → `DONE_FOR_SLICE`/`PASS`（#136 + #142）；`就座交互` 行 ⑧ 已通过 → `DONE_FOR_SLICE`；`角色动作呈现` 行 `REGRESSED` 判据失效 → `PASS`；§三 KI 行因果更正；§四 「无 Unity Editor」阻塞解除并拆出 P4d 的真实前置 #135；测试计数不再复述、改为引用构建与测试 §四（#130）；补两行「动作集机器源」（#138）与「locomotion 连续混合（契约 B）」（#140）；`Core → Unity 适配层` 行按 #155 改判 `FAKE`→`NONE`；持椅交互行加 owner 目视不通过 + 该线**暂停**）*
