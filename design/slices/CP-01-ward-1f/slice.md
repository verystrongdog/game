...
| **动作描述口径（人机协作接口）**（✏️ 2026-09-14 新增） | **`ACCEPTED`** | **`NONE`** | `ISOLATED` | `UNKNOWN` | 设计面 = [动作描述口径.md](../../presentation/%E5%8A%A8%E4%BD%9C%E6%8F%8F%E8%BF%B0%E5%8F%A3%E5%BE%84.md)（D1–D10 经 owner 逐题接受；U2–U7 为诚实清单）。**缺口**：全仓 `grep 回显卡\|自由度表` 在 `code/` 下**零命中** ⇒ 口径**从未在真实动作回路里被消费**；ε 与 52 根无约束变形骨的轴向语义**均未测** → [#160](https://github.com/verystrongdog/game/issues/160)（ε 实测）· [#161](https://github.com/verystrongdog/game/issues/161)（轴向语义表）· [#162](https://github.com/verystrongdog/game/issues/162)（桥内直跑）· [#163](https://github.com/verystrongdog/game/issues/163)（回归样本）|
