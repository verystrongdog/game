...
| **Unity 动画约束与接触修正**（✏️ 2026-09-14 新增） | **`CANDIDATE`** | **`NONE`** | `ISOLATED` | `UNKNOWN` | [动画处理能力对照实验.md](../../presentation/动画处理能力对照实验.md)（共同样本与观测口径——**草案 `CANDIDATE`**，§七 五项待 owner 裁定）+ [动作库规格.md](../../presentation/%E5%8A%A8%E4%BD%9C%E5%BA%93%E8%A7%84%E6%A0%BC.md) §四·戊 #11（副手 IK 属**已暂停**的持椅线，不作本条依据）。**现状**：Animation Rigging `1.4.1` 已入库（提交 `dcc0073`）、`ActionLab` 场景已有**惰性**接入点（`RigBuilder` + 左臂 `TwoBoneIKConstraint`，权重恒 0，提交 `c42c1cd`）与 2 条真场景能力断言（提交 `cb3523a`，读数 0.0 mm / 对照 187.1 mm，见 `code/unity/README.md` §二·O）——但那是**接入点不是能力**（无消费方、不产生行为），故 `Implementation` 记 `NONE`：右手/双脚/根目标、可关键帧权重与接触修正烘焙路径均不存在 → [#157](https://github.com/verystrongdog/game/issues/157)。⚠️ 本行 `Design` 为 `CANDIDATE` ⇒ 校验器 I4 对 #157 的 `Implementation NONE → PARTIAL` 声明**按其规则判 FAIL**——这是"设计未 `ACCEPTED` 不得声明实现迁移"的**正确信号**，不是登记错误；owner 裁定共同样本文档 §七 后本格改 `ACCEPTED`，该 FAIL 随之消失。🔧 2026-09-14 登记：此前本行缺失，致 I4 对该能力「无法判定」 |
...
---
*创建: 2026-09-12 | 更新: 2026-09-14（🔧 四轴表补一行「Unity 动画约束与接触修正」（#157）——此前未登记致 I4 无法判定；该行 `Design` 为 `CANDIDATE`（共同样本文档 [动画处理能力对照实验.md](../../presentation/动画处理能力对照实验.md) 尚是草案、§七 五项待 owner 裁定），故 I4 对 #157 的实现迁移声明按规则判 FAIL，属正确信号。此前批次：2026-09-13 四轴表与 §三/§四 回填：`Unity 资产身份` 行 `NONE`/`UNKNOWN` → `DONE_FOR_SLICE`/`PASS`（#136 + #142）；`就座交互` 行 ⑧ 已通过 → `DONE_FOR_SLICE`；`角色动作呈现` 行 `REGRESSED` 判据失效 → `PASS`；§三 KI 行因果更正；§四 「无 Unity Editor」阻塞解除并拆出 P4d 的真实前置 #135；测试计数不再复述、改为引用构建与测试 §四（#130）；补两行「动作集机器源」（#138）与「locomotion 连续混合（契约 B）」（#140）；`Core → Unity 适配层` 行按 #155 改判 `FAKE`→`NONE`；持椅交互行加 owner 目视不通过 + 该线**暂停**）*
