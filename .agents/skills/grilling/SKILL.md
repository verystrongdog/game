...
3. **质量门禁**（v2 新增）：运行自动化校验脚本（**⚠️ 2026-09-17：本项目已撤销全部门禁，那些校验脚本与其注册表、CI job 一并删除——本步在本项目内没有可跑的脚本**；等效检查只有人工：cross-ref grep + grilling 产物完整性目视核查，这与本流程停用前的"当前阶段"做法相同）。输出关闭总结（决策表 + 受影响文件 + 写入验证 + 校验结果）。
