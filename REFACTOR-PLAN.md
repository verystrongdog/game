...
- [ ] 5 个正确性断裂全部修复，`tools/validate_*.py` 全部退出码 0
...
| **D10** | **删除 `src/YouAreNotTheFish.Core.Unity/` 草稿**（从未构建成功，Q6b 定案时重建） | 用户裁定 |
...
3. **修 2 处新发现的三方矛盾**（勘察副产物，优先级高）：
   - **PAD**：`规则/技能树系统/脑功能层级模型.md:9` 把 PAD 列入已废弃，但注册表 active。真相需区分——`3D可视化/脑图谱数据管线.md:3,20-23,142-144` 记载技能坐标**已从 PAD 概念空间迁移至 MNI 解剖坐标**，故 PAD 在**技能定位**上退役、在**情绪表示**上仍活跃。正典 L9 的措辞需改为「PAD 作为技能定位空间已废弃」。
...
|---|---|---|
| `规则/` | `design/rules/` | 39 |
| `实体/` | `design/entities/` | 39 |
| `空间/` | `design/space/` | 99 |
| `事件/` | `design/events/` | 6 |
| `呈现/` | `design/presentation/` | 1 |
...
| `参考/` | `reference/` | 220 |
| `参考/废弃/` | `design/archive/deprecated/` | 11 |
| `垃圾桶/` | `design/archive/trash/` | 27 |
...
| 迁出对象 | 现有消费方 |
|---|---|
| `.scratch/csharp-*/design/spec.md` × 10 | ~90 处 `src/` XML 注释以 `spec@v1.1` 引用 |
...
**顺带修出的真实数据错误**（原被回退掩盖）：`实体/疾病目录/躯体症状障碍.md` 的 `父类` 字段写成自身「躯体症状障碍」，应为「躯体症状」——回退靠 rglob 匹配到文件自身而"通过"。全量核验 26 个疾病文件，仅此 1 处。
...
| 项 | 结果 |
|---|---|
| 新建 `规格/` | 49 文件（引擎 13 / 素材 34 / 数据源 3）——被代码或正典消费的承重内容 |
| 新建 `设计归档/grilling/` | 51+ 目录——各轮决策记录、任务计划、工作稿、实现记录 |
| `.scratch/` 受控文件 | 341 → **0**（移出版本控制；本地 510 文件 / 47M 保留） |
| `.scratch/` 引用 | 310 处重写；仍指向存在目标的引用 **0** |
| `呈现/` | 1 文件 → 5 条目（含 `3D可视化/` 整目录搬迁） |
| `docs/` | 混杂 17 文件 → 写作与引用规范 / 决策树/ / 设计框架-六维状态 / 维度/ / agents/ |
| `docs/决策树.md` | 4519 行单文件 → `docs/决策树/` 6 片段 + README（**93 个 H2 逐一比对完全一致**） |
...
| 0.5 | ~~删垃圾受控文件：4 个 `PDF_page_*.png~RF*.TMP`~~ → **⚠️ 已修正：这 4 个文件不是垃圾**，而是 `raw/参考图/` 序列中第 26-29 页的**唯一副本**（正式文件名缺失，`.TMP` 是 Windows 提取工具未完成改名的残留名）。已 `git mv` 改名为 `PDF_page_26..29.png`，序列修复为 01-34 完整 | `空间/治疗中心建模/references/raw/参考图/` |
| 0.5b | 删除 `data/brain_regions_backup_20260712_—.json`（文件名含乱码破折号，零文档引用，已 grep 确认唯一"引用者"是本计划文档） | — |
| 0.5c | 删除空目录 `练习/`（0 文件、未跟踪、零引用） | — |
| 0.6 | 删除 `参考/文献/` 的 7 组 md5 重复 PDF + 1 组跨目录重复 | 省约 15 MB |
| 0.7 | 删除 `参考/会话存档/` 的 26 个 `raw.*_tmp.html`（~7 MB，已 ignore） | 抓取中间产物 |
...
| 1.7 | 修 2 个断裂 sim 脚本（import 目标已进垃圾桶） | `sim/extract_formulas.py:10`、`sim/test_evolved_cog.py:72` |
| 1.8 | 修 `validate_spatial.py:77` 过时断言（`>=6` 设计节点，实际 0，因 #25 已删） | `tools/validate_spatial.py:77` |
| 1.9 | 修 `review-plan/SKILL.md:280` 链接层级（`../../` → `../../../`） | `.claude/skills/review-plan/SKILL.md:280` |
| 1.10 | 修 `unity/README.md:19` Unity 版本号（6000.0.83f1 → 6000.5.2f1） | `code/unity/README.md` |
| 1.11 | 裁定 §4.3 的 PAD / 意志力 三方矛盾并同步两侧 | `脑功能层级模型.md:9,567` + `term_registry.json` |
| 1.12 | 7 项 `validate_params` C1 失败：均为提取器把不同语境当同名参数的**假阳性**，修提取器或将 `L`/`k`/`ΔI`/`β(t)`/`β₀`/`ε`/`—` 加入例外清单 | `tools/validate_params.py` |
...
> **注意**：`tools/validate_trash_isolation.py` 只报了 2 处，因为其 `FILES_IN_TRASH`（L22-30）硬编码 7 个 L0-L6 文件、`EXEMPT_DIRS` 豁免 `.scratch/`、且只扫 `.md`（L113,128-134）——**它检查不了 `data/`、`reference/`**。Phase 1 应同时扩展该脚本的覆盖。

**验收**：`tools/validate_*.py` 全部退出码 0；死链扫描 0 失效。
...
| 2.8 | 合并 `空间/治疗中心建模/` 的 8 个占位 README（336 行，6 个仅 9-17 行）为 1 个 | — |
...
| # | 动作 |
|---|---|
| 3.1 | 写一次性迁移脚本 `tools/migrate_paths.py`：按 §5.2 映射 `git mv` 全部目录 |
| 3.2 | 同一脚本批量改写引用：2619 处 md 引用 + `tools/` 内 90 处硬编码路径 + `src/` 约 90 处 XML 注释 + 12 个文件名含中文的 md 内部链接 |
| 3.3 | 手工修 4 处**无法正则化**的路径基准（见 §7.1） |
| 3.4 | 改写 `tools/md_utils.py` 的 `SCAN_ROOTS` / `EXCLUDE_DIRS` / `EXCLUDE_FILE_KEYWORDS` |
| 3.5 | 逐目录验证后分组提交（按主题分批，不混 commit） |

**验收**：`tools/validate_cross_refs.py` 0 死链；`dotnet test` 通过；`python3 code/tools/run_all_checks.py` 全绿。
...
| 4.8 | 改 `tools/sync_issues.py` 的 `TYPE_LABELS` 移除 `"grilling"` | — |
| 4.9 | 改 `.github/ISSUE_TEMPLATE/grilling.md` 与 `docs/维度/管线.md`、`docs/维度/规则.md` 的索引标题（去掉 grilling 措辞） | — |
| 4.10 | 修正 `docs/agents/issue-tracker.md` 与 `domain.md` 的**死规范**：issue-tracker 描述的 `.scratch/<slug>/spec.md` + `issues/NN-slug.md` 实测**使用 0 次**；domain.md 要求的 `CONTEXT.md` + `docs/adr/` **不存在却被引用 10 次** | 决策点 D7 |
...
| # | 动作 |
|---|---|
| 5.1 | `tools/validate_*.py` 全部退出码 0 |
| 5.2 | 死链扫描 0 失效（活跃设计文档） |
| 5.3 | `git gc` 后报告 `.git` 体积对比 |
| 5.4 | 重写 `README.md` 导航表与 `项目总览.md` 使其匹配新结构 |
...
| 位置 | 内容 | 修法 |
|---|---|---|
| `tools/md_utils.py:14` | `ROOT = Path(__file__).parent.parent` | 移到 `code/tools/` 后 → 需 `parent.parent.parent`，或改为向上查找 `.git` 的 `find_repo_root()` |
| `tools/md_utils.py:21` | `SCAN_ROOTS = ["规则","实体","空间","事件","呈现","管线","docs","参考"]` | 改为 `["design","reference"]` |
| `tools/md_utils.py:24-28` | `EXCLUDE_DIRS` 含 `"垃圾桶"`,`"参考/废弃"`,`"参考/书籍"` | 同步改名 |
| `tools/md_utils.py:31` | `EXCLUDE_FILE_KEYWORDS` 含 `"已废弃"`,`"决策树.md"` | 同步改名 |
| `tools/validate_cross_refs.py:99` | 硬编码 `实体/疾病目录/_父类/{parent_name}.md` | 改为 `design/entities/diseases/_parent-classes/` |
| `src/…Tests/Data/GameDataLoaderTests.cs:9` | `AppContext.BaseDirectory` 上溯 **5 层** 到 `data` | `src/` → `code/src/` 后需 **6 层**；并删除 L20 的 `/home/dog/game/data/...` 绝对路径 fallback |
| `unity/Assets/Scripts/DemoSolver.cs:30-44` | 手抄 `CalibrationConfig` 常量 | Phase 1 改 `NpcHp` 时同步；建议标注为 demo 占位或直接删除 |
...
| `sim/` 扁平 import | `sim_consciousness_v7_components.py:33,36` 等 | `sim/` 不能改成包结构，除非同步改 import |
...
| Windows 绝对路径 | `MixamoSetup.cs:16`、`tools/unity-cli/uc.sh:5`、`process_hospital_atlas.py:245` | 跨机不可复现 |
| `tools/` 硬编码脑区数 | `brain_atlas_to_blender.py` 自述「57 脑区」（现 69） | 工具未随 #25 更新，重跑产出错数据 |
...
all_obj/                          278M  ← 中间网格
```
`tools/brain_atlas_to_blender.py` 重建的是脑图谱底图，**不是技能树布局**。这是本次勘察中**唯一的数据丢失风险**。
...
| 不删除 `design/archive/` 任何内容 | grilling 归档保留；`参考/废弃/情绪系统/NRC-VAD-Lexicon.tsv`、`Ratings_Warriner_et_al.csv` 是 PAD 空间的原始数据源 |
| 不拆仓 | 用户已定单仓 |
| 不删除 `docs/agents/` 的 4 份文档 | 保留文件，但修正其与现状不符的规范（Phase 4.10） |
...
| **D7** | `docs/agents/issue-tracker.md` 与 `domain.md` 是死规范 | **保留原样**，不改写不删除 | 取消原 Phase 4.10 |
...
| **D10** | `src/YouAreNotTheFish.Core.Unity/` 草稿 | **删除** | Phase 0.8 |
