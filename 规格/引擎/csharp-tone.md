...
本规格的决策依据 = [任务issue 01](issues/01-tone-spec.md) D1-D12（其「数据实测」表为本 spec 全部计数与锚点的出处，2026-08-13 python 实测 JSON，非凭记忆）。
...
| B2 | **tone 解析解**：[运行时状态模型](../../../规则/技能树系统/运行时状态模型.md) §5.2/§7.1 为 Euler → 解析指数解（plan §十二-2 已裁定） | plan 审计数值证明 Euler 在此参数下失真；τ_tone 0.3-1.5 无 underflow 风险（k_t ≥ 0.0357） |
