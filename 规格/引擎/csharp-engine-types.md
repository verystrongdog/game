...
| v1.1 | 2026-08-14 🔧 修正（L2 语义，csharp-events spec@v1.2.2 §二 2.1 承接）：StatusChangeEvent +3 float 字段（SanBefore/SanAfter/SanMax——C1 guard 需 SAN 跨越前后值，任务issue Q1=A）。Kind=Downed 时 3 字段语义未定义（可不填，默认 0f）。CalibrationConfig +EndurancePassivePenalty=1 [NEW]（A4 expected 公式分母，Q3=A）。本 spec 字段表对应位置以 csharp-events spec 为准 | csharp-events 全量审计链（含 AC-13 回执：现有 types 测试零改动通过，228/228） | 随 csharp-events 审计 |
| v1.1 | 2026-08-14 🔧 修正（L2 语义，csharp-flow spec@v1.2 §四 承接）：CalibrationConfig +MaxRounds=50 [NEW]（回合上限——结束条件 §5.5，待校准）。本 spec 对应位置以 csharp-flow spec 为准 | csharp-flow 全量审计链（含 AC-14 回执：现有 types 测试零改动通过，261/261） | 随 csharp-flow 审计 |
