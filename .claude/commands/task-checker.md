...
1. **维度① 格式规范**：对照 `CLAUDE.md §md 文件格式规范`——文件头摘要/文末关联/目录（>200 行）/单 H1/中文编号小节/废弃标记/参数速查表
2. **维度② 术语与正典一致性**：运行 `python3 tools/list_deprecated_terms.py` 获取完整 deprecated 清单（**勿用记忆/硬编码**），扫描目标文件；并核对注册表定义误用、与决策树冲突
3. **维度③ 交叉引用完整性**：复用 `tools/validate_*.py` 核验活跃目标；手动核验任务 md 自身链接（`.scratch/` 不在脚本扫描范围）；检查垃圾桶隔离
4. **维度④ 数学语言充分性**：对照 `docs/agents/math-language-writing.md`——模糊量词/无符号数值/阈值模糊/枚举未受控等，给出公式化改写建议
