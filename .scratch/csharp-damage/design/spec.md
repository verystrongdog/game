...
> 物理/精神伤害结算（命中判定 + 低 SAN 穿透 + 防御/回避修正 + 一位小数量化）——csharp-engine step 8 引擎模块。版本: v1.0
...
- 现有 types 测试（ParticipantStateTests/CombatEventsTests）以 int 字面量断言——xUnit `Assert.Equal(3, e.DamageDealt)` 经 int→float 隐式转换继续成立；AC-18 为回执验证。
...
- 未定义行为（文档化，不防御）：NaN/Infinity 输入、负 baseDamage（调用方职责）。
...
```
raw = 2f × (1f + motivationMod) − (float)endurance        // motivation 不 clamp（负值降伤害，下限由 max 兜底——plan §4.6 字面）
...
- 未定义行为（文档化，不防御）：NaN/Infinity 输入、ratio ∉ [0,1]、负 endurance。
...
- `MathF.Round(float, int, MidpointRounding)` 委托 double 精度实现（.NET 8 文档行为：`(float)Math.Round((double)x, digits, mode)`）——无 f32 ×10 乘法误差。**实现测试以真实 MathF.Round 输出钉锚点**（任务issue 第二轮为设计级仿真）。
- Floor1 的 ×10f 步在 f32 域（利用 f32 舍入修正——如 1.3f×10f 精确舍入到 13.0f），scale-back 用 **`/ 10f` 精确除法**落回 f32(0.1k) 网格点——`× 0.1f` 乘法会引入 1e-7 级离格（实测：1.3f→1.3000000715、0.95f→0.9000000358，逐位锚点会挂）。
- 每步结算产物立即量化 → 状态恒在 0.1 网格的 f32 最近值 → 跨回合无漂移（实测 #13）。
...
| L0 回避命中惩罚 | L0EvadeHitPenalty | 0.10f | 回合战斗流程 §6.2（L0 回避 −10%）；plan 修复 #10（无条件） |
...
6. **常量引用**：12 常量在 CalibrationConfig——DamageCalculator 不内置数值（构造注入 cal；AC-17 逐常量断言默认值）。
...
| AC-2 | 物理全乘区（一位小数核心锚） | (4, 0, 0.5, 1.0, 1.0f, false) → **10.0f** 逐位（实测 #5）；(4, 0, 0.25, 0.5, 0.8f, false) → **5.6f 逐位**（任务issue 实测 A——整数世界 floor 5 / round 6 全失真，一位小数保留） |
...
| AC-5 | 命中边界 | SeqRng 注入 roll 序列 [0.0, 0.74, 0.75, 0.74999994, 0.999] → [hit, hit, **miss**, hit, miss]（阈值 0.75f 严格小于且 f32 精确；0.74999994f = 次邻 → hit） |
...
| AC-13 | 穿透边界（严格小于 + 互斥 + 最低档救活） | inner 1.0、gate 1.0f：ratio **0.30** → san **1.0f**（不触发）；0.2999… → **1.3f**；**0.15** → **1.3f**（tier1 非 tier2）；0.149 → **2.0f**；穿透在最低伤害生效：1.0f×1.3 → **1.3f**（整数世界两法皆 1——一位小数救活） |
| AC-14 | 穿透 + 量化组合 | (2, 0, 0, 0.299, 1.0f) → san **2.6f**、hp **1.3f** 逐位（实测 H——floor1 经 /10f 精确落回 f32(1.3)）；(2, 0.25, 1, 0.299, 1.0f) → san **1.9f**、hp **0.9f** 逐位（inner 1.5×1.3 = f32 精确积 **1.94999993f**（低于 1.95）→ Round1 → 1.9——量化域锚，2026-08-13 f32 复测）；(2, 0, 0, 0.149, 0.9f) → san **3.6f**、hp **1.8f**（实测 J） |
...
| AC-17 | CalibrationConfig 12 新常量 + 4 模板 | 默认值逐常量断言：4 / 2 / 0.85f / 0.10f / 0.5f / 1.0f / 0.5f / 0.30f / 1.3f / 0.15f / 2.0f / 0.1f（§四 4.1）；PlayerHp 50f / PlayerSan 80f / NpcHp 15f / NpcSan 60f（float 类型断言——L2） |
| AC-18 | L2 类型变更回执 | 现有 types 测试（ParticipantStateTests/CombatEventsTests）在新 float 类型下全绿（int 字面量隐式转换）；新实证：PhysicalDamageEvent.DamageDealt = 5.6f 存读回逐位；CreateDefault(50f, 80f) → Hp **50f** 逐位 |
...
2. **数值断言全部实测**：全部锚点（10.0/5.6/1.95→2.0/1.5/0.7/2.6/1.3/3.6/1.8/阈值 0.75 与次邻）经 python f32 实测（任务issue 01 实测表第一轮 #5-#9 + 第二轮 #10-#14 + spec 写作期链式复算）——禁凭记忆写。
...
| L0 回避命中惩罚 | L0EvadeHitPenalty | 0.10 | 回合战斗流程 §6.2；本 spec §四 |
...
| 版本 | 日期 | 变更 | 触发 | 审计范围 |
|------|------|------|------|----------|
| v1.0 | 2026-08-13 | 初稿（§一~§八 + AC-1~18 + B1-B6）；审计前自修正 2 处：AC-14 第二行锚点 f32 复测（1.5×1.3 链积 1.94999993f→san 1.9f/hp 0.9f，原 2.0/1.0 系仿真 float64 泄漏）、Floor1 scale-back ×0.1f→/10f（×0.1f 乘法离格实测） | 任务issue 01（Q1=A/Q2=A/Q3=D 裁决后；设计文档写回 commit 112616c/bddae18） | 全量审计 |

---
*创建: 2026-08-13 | 更新: 2026-08-13 | 版本: v1.0*
*关联: [任务issue 01](issues/01-damage-spec.md), [csharp-engine plan §4.6/§六](../../../csharp-engine/design/plan.md), [核心机制](../../../规则/核心机制.md) §4.2/§4.3, [基础行动设计](../../../规则/技能树系统/操作层/基础行动设计.md) §二/§四, [回合战斗流程](../../../规则/回合战斗流程.md) §6.2, [运行时状态模型](../../../规则/技能树系统/运行时状态模型.md) §6.5*
