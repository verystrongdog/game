...
- Q3=D 连带决策：D13 量化函数（MathF.Round(x,1,AwayFromZero) 委托 double 精度 + MathF.Floor(x×10f)×0.1f，每步结算后量化防 f32 漂移）、D14 **L2 跨 feature 类型变更 18 字段 int→float**（ParticipantState 4 + PhysicalDamageEvent 6 + MentalDamageEvent 8——types 文件当前零消费者）、D15 设计文档写回（基础行动设计 行动2/§四 + plan §六 + 核心机制 §4.2 + term_registry）、D16 呈现层显示规则不覆盖
- 关键实测（第二轮）：三敏感值全保留（5.6/1.5/2.6）；**最低档穿透救活** 1.0×1.3→1.3（整数世界两法皆 1）；inner m=0.25→1.5（动机不再被取整吞掉）；0.7 网格点 f32 存储 0.699999988079071

## Fog

- 任务issue 数据实测 ✓ → Q 裁决 ✓（Q1=A/Q2=A/Q3=D 一位小数）→ 设计文档写回 → spec v1.0 → 全量审计 → 修复升版 → Δ审计 → sign-off → 工作issue 01 实现 + 自审（含 18 字段类型变更）→ feature 闭合
