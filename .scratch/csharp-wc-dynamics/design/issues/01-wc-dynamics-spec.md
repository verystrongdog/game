...
| e^(−Δ/τ) 解析解系数（Δ=1，τ 三档） | fast τ=0.01 → e^(−100)=3.72e-44（**float32 下溢 = 0**）；medium τ=0.05 → e^(−20)=2.06e-9；slow τ=0.15 → e^(−6.67)=**1.27e-3** |
| σ 锚点（v=1, θ=0.5） | σ(0.5)=0.5（不动点 x=σ(x) 验证）；σ(0)=**0.3775**——皮层动力学 §4.2 叙述「σ(0)≈0.006」实测有误（正确 0.3775） |
| b=0 固定点预览（真实 W 归一化矩阵，a(0)=0.10，30 回合迭代 a←σ(W·a)） | 第 2-3 回合即收敛（max\|Δ\|=0.00e+00）；终态 mean=0.4623、max=0.4995；21 排除节点（W 行=0）终态 = σ(0) = 0.3775 |
...
plan §十一 step 4 = 「WcDynamics → M1 静息 trace 里程碑」，但 M1 定义（plan §八）要求 **tone=baseline、s=0 运行 30 回合**——tone=baseline 意味着需要 b_j = Σ tone × 脑干权重，而 b_j 生产方 CorticalBias 是 **step 5** 的产物。step 4 若执行 M1 只有两条路：(a) b=0 跑——测的是理论玩具吸引子（实测 mean=0.4623，21 排除节点被 σ(0)=0.3775 拉低），不是 plan §八 要的「真实静息态」；(b) 测试内临时复制 CorticalBias 公式——违反文件所有权（step 5 会重复实现）。

**AI 建议：M1 推迟到 step 5**——CorticalBias GREEN 后立即执行真实 trace（tone=baseline、s=0、30 回合），step 4 只交付 WcDynamics.Step + 单元测试（含 b=0 固定点收敛性质测试作铺垫）。plan §十一 step 4 行加一句「M1 随 step 5 执行」注释（L1）。

### Q2: 慢节点叙述与解析解数值矛盾——记录偏差并延后文档修正？

实测 e^(−6.67)=1.27e-3 → 即便 τ=0.15 慢节点，一回合收敛 99.87%，跨回合「保留历史」的幅度可忽略。皮层动力学 §4.3 的「slow 节点跨回合平滑过渡、保留历史」叙述在解析解下不成立（Euler 下也不成立——plan §十二-1 审计已证明 Euler 是 bang-bang 振荡而非平滑）。

**AI 建议：是**——spec 记偏差声明 B（推论：动力学实质为固定点迭代，与 plan §十二-5 静息吸引子偏差同源）；文档叙述修正并入 plan §十三-8 的 Euler 一致性清扫（皮层动力学 §4.3 已在清扫清单内），不单独行动。顺带把「σ(0)≈0.006 → 0.3775」笔误一并列入清扫清单（σ 段落 §4.2 不在清扫清单内，需显式追加）。
...
| D6 | **偏差声明 B**：解析解推论——全节点单回合收敛 ≥99.9%，动力学=固定点迭代；「慢节点跨回合保留历史」叙述不成立。实现照解析解，文档叙述修正延后 | 数据实测表 e^(−Δ/τ) 行；plan §十二-1 已裁定解析解，此为推论声明；修正并入 §十三-8 清扫（见 Q2） |
...
- [ ] spec.md v1.0（§一~§七 + 变更日志，AC 覆盖 plan §十 WcDynamics 行）
