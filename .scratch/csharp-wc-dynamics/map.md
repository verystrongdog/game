...
- 数据实测已完成（任务issue「数据实测」表，2026-08-13 python）：12 bimodal / e^(−Δ/τ) 3.72e-44→0（float32 下溢）、2.06e-9、1.27e-3 / σ(0)=0.3775（文档 0.006 有误）/ b=0 固定点 2-3 回合收敛、mean 0.4623
...
- Q1：M1 静息 trace 里程碑归属（AI 建议推迟到 step 5——b_j 生产方在那里）
- Q2：慢节点叙述矛盾处置（AI 建议偏差 B + 文档修正并入 plan §十三-8 清扫）
- spec v1.0 → 审计 → sign-off → 实现（待追问裁决后启动）
