...
| 修复点 | 受影响消费者 | 复查状态 |
|--------|------------|---------|
| AC-11 计数 82→50 | §五 C5、§七自检清单数字 | ✅ Δ审计通过（三处一致，无残留 82 活数字） |
| AC-8 分布 27/30/12 | §二 Step 6 实测段 | ✅ Δ审计通过（Step 6/AC-8/§七 三处一致） |
| B2 数字修正 | 任务issue D1/数据实测表（并集口径行保留为并集标注） | ✅ spec 侧通过；issue 并集行已补标注（Δ审计后 L1） |
| AC-7 命名粒度 | 测试实现方式（工作issue） | ✅ Δ审计通过；L1 补 Step 4.3 解析规则引用（测试方式文字） |
| Step 4.3 理由句 | 任务issue D4 | ✅ spec 侧通过；issue D4 行已补修正（Δ审计后 L1） |
| B1 修正清单补全 | 审计后 L1 文档修正任务 | ✅ 5 处章节/行号全部核实属实 |
| 任务issue「CC-only」行 | spec §一（「issue 表为本 spec 出处」声明） | ✅ 三口径实测吻合；L1 补「有 CC 边」限定（另有 5 brainstem dk 两边皆无） |

## Δ审计（spec v1.1，2026-08-13）

> 方法: workflow 2 专家（修正验证 + 半径扩张复查）| 范围: 变更章节 + 半径扩张（不全量重审）

### 修正验证专家结论: conditional（0 error / 2 warning）

- **F1-F3、F6-F12 全部已修复**（独立 python 实测确认）：50 = Σ_dk n(n−1) 吻合、27/30/12 继承后分布吻合、Cerebellum-Cortex 幸存 CC 6+PP 12 与三 dk 并集 CC 40+PP 51 双口径吻合、B1 清单 5 处原文核实属实、CC-only 三口径吻合、段号 §4.3、绝对容差 1e-5 属实（全 float32 流水线实测偏差 1.18e-8 < 声明的 2e-7）、变更日志 12 条 AC、Engine/ 目录表述。
- **F4 ⚠️（warning）**：AC-7 测试方式「经 fid→dk 映射展开」方向性含混（fid→dk 无法展开 dk→fids；'Pericalcarine' 为 fid 名而 dk 键为小写 'pericalcarine'）→ **已按 L1 修正**：测试方式补「锚点标识符按 Step 4.3 端点解析规则展开（graph_nodes 键优先 → fid→dk 兜底）」（变更日志 v1.1 🔧 行）。
- **F5 ⚠️（warning）**：任务issue D4 依据行残留旧表述（「其 dk 单 fid」对 Caudate 为虚假声称）→ **已按 L1 修正**：D4 行补「Caudate（2 fids）∈ CSTC 排除集，排除检查先于解析」。
- 附带 info 已一并处置：F3b（issue 并集口径行补标注）、F10（哨兵注扩展至 AC-3/5/12）。

### 半径扩张复查专家结论: pass

- R1（info）：CC-only 口径 1 字面全量实为 11（另有 5 brainstem dk 两边皆无）→ 已按 L1 补「有 CC 边」限定。
- R2（info）：map.md 状态同步——审计运行期间已由提交 `fbd5078` 解决（Status/AC 数/D1-D8/Fog 均已更新）。
- R3（info）：report.md 补 Δ审计段（即本节）+ 半径扩张复查状态更新（见上表）。
- R4（info）：任务issue「产出」清单待 sign-off 时统一勾选。

### 最终审计结论

- [x] **有条件通过**——0 error；2 warning（F4/F5）均为表述/文档同步类缺陷，已按 L1 通道直接修正（变更日志 v1.1 🔧 行，免第三轮审计）。数值、公式、接口契约全部经两轮独立实测确认。进入人类复核（sign-off）。
