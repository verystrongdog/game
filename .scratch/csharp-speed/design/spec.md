...
> 三成分速度排序（察觉/决断/执行）与同速掷硬币破平——csharp-engine step 7 的两个引擎模块。版本: v1.0
...
| Decision | float | [0, 1] | 决断分——mean(c_loop, 3 环路)；**无回落**（c_loop≈0 即决断慢，与 csharp-cstc「c=0 → gate 0.843 门控抑制」一致） |
| Execution | float | [−0.1, 1.5] | 执行分——mean(a(Precentral), a_SD1_somatic) + M1 惩罚；上限 1.5 来自 a_SD1 解析界 [0, 2]（csharp-cstc spec §四），下限来自惩罚 −0.1 |
...
静态工厂 `SpeedWeights.FromCalibration(CalibrationConfig cal)` → (SpeedW1, SpeedW2, SpeedW3)。权重只在 §3.1 合成层（非成分内部）——运行时状态模型 §8.1 已指针化（2026-08-13 step 7）。
...
- 异常：null 输入 → ArgumentNullException；`a.A` 长度 ≠ 69 → ArgumentException（AC-12）。
- 未定义行为（文档化，不防御）：NaN/Infinity 输入。

**ComputeScore**：`speed = W1×Perception + W2×Decision + W3×Execution`（回合战斗流程 §3.1；分量归一化 mean 见 B2）。speed 为**序数值**——绝对量无意义（§3.1「速度只需要一个序数结果」），值域等权下 ≈ [−0.0333, 1.1667]。
...
- **破平**（§3.4「同速时掷硬币，每回合重新破平」）：每个平局组内 Fisher-Yates 洗牌。**算法写死**（RNG 消费序契约，测试镜像依赖）：
  1. 组内索引保持输入升序；
...
2. **数据流时序**（运行时状态模型 §7.1）：`salience` 与 `gurney` 必须来自**本回合同一次** `CstcGating.Step` 的返回值（步骤 4-5 → Phase 3 速度重算）——不跨回合混用（编排层职责，本 spec 只声明输入语义）。
...
| AC | 验收标准 | 锚点/判定 |
|----|---------|----------|
| AC-1 | 构造解析 5 行索引（4 察觉 + Precentral），行序 = RegionIds | 测试从 RegionIds 现算期望索引（不硬编码行号——数据漂移哨兵）；未命中名 → KeyNotFoundException |
| AC-2 | 察觉 = mean(4 节点)，无回落 | 全 0.7 → 0.7；混合 (0.7,0.7,0.7,0.6751733) 精确均值（测试侧现算） |
| AC-3 | 察觉逐节点回落（Q2=A） | (0.7, 0.7, 0.7, 0.05) → **0.6937933**（0.05→ACC 静息 0.6751733）；全回落（4 节点 <0.15）→ **0.64263445**（4 静息均值）；端点：恰好 0.15 → 不回落（≥ 阈值）；锚点 1e-5，回落常量 = §四 4 值 |
...
| AC-10 | BuildOrder 全平局：破平 = §三 3.2 Fisher-Yates | 测试镜像同算法 + 同种子 DeterministicRng → 期望排列**逐元素相等**（RNG 消费序契约）；结果 ⊆ 输入索引排列；同种子两次调用 → 相同 |
| AC-11 | BuildOrder 混合平局组 | [0.8, 0.5, 0.5, 0.2] → 结果[0]=0、结果[3]=3、中间两位 = {1,2} 的洗牌；固定种子 → 精确序 |
| AC-12 | 契约防御 | ComputeComponents：a/salience/gurney null → ArgumentNullException；a.A 长度 {0,68,70} → ArgumentException；BuildOrder：scores/rng null → ArgumentNullException |
| AC-13 | 输入不可变 + 确定性 | ComputeComponents 输入快照不变 + 输出不共享输入数组（NotSame）+ 重复调用逐位相等；BuildOrder 同种子重复调用逐位相等 |
| AC-14 | 边界 | 空 scores → 空数组（不消费 rng）；单元素 → [0]（不消费 rng）；a_SD1=0 → 执行 = mean(Precentral, 0)；a_SD1=2.0（解析界上限）→ 执行 = 1.5（未防御）；c_loop 全 0 → 决断 0 |

## 七、本 spec 自检清单

1. **数学公式逐项对照**：三成分公式/逐节点回落/加权合成与 回合战斗流程 §3.1-3.3 + plan §4.5 逐字一致（含 mean 归一化 B2、Putamen 代理 B3）。
2. **数值断言全部实测**：全部锚点（§四 4 常量、静息三分量、0.71175098、0.6937933、RED 0.5/0.45/1.0）经 python f32 复算（任务issue 01 实测表）——禁凭记忆写。
3. **行序契约明确**：canonical = RegionIds 唯一（§五 1）；AC-1 数据推导断言承接。
4. **接口注释先行**：构造 + ComputeComponents + ComputeScore + BuildOrder 的 XML doc 含职责/输入/输出/异常/未定义行为/来源/偏差标注（"设计两次"约束，plan §十一）。
5. **边界值覆盖**：阈值端点 0.15 / 静息 / 全 0 / a_SD1=0 与 2.0 / 空与单元素 scores / 全平局 / 混合平局。
...
| B2 | 分量归一化 mean | 回合战斗流程 §3.2/§3.3 求和形态（察觉 4 节点和、执行 2 节点和） | 统一取均值 | plan §十二-8：三成分量纲可比（0-1）、等权 1/3 语义成立；求和形态下察觉实际权重 4× |
| B3 | Putamen 代理 | 回合战斗流程 §3.3「a(Putamen)(t)」——Putamen 为 CSTC 节点无 WC a(t) | a_SD1_somatic（Q3=A，plan §十三-4 已裁决） | SD1 = 壳核直接通路（D1 MSN）的 Gurney 结构同构映射，非任意代理 |
...
*关联: [任务issue 01](issues/01-speed-spec.md), [csharp-engine plan §4.5](../../../csharp-engine/design/plan.md), [回合战斗流程](../../../规则/回合战斗流程.md) §3.1-3.5, [运行时状态模型](../../../规则/技能树系统/运行时状态模型.md) §7.1/§7.3/§8.1*
