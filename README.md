...
> **叙事驱动的 3D 心理 CRPG**（以神经科学为骨架、以「意识外显效应」为世界观）。主循环是**对话 / 调查 / 认知检定 / 承诺·线索·自问**；回合制心理对抗是**并列解决路径之一**，不是必经环节。**设计文档是 primary artifact**；`code/` 是验证与实现工具。
>
> 🔧 **定位裁定 (2026-09-16 · [#181](https://github.com/verystrongdog/game/issues/181) 选项 A)**：由 owner 在「叙事优先 / 叙事与战斗并重 / 战斗优先」之间选定**叙事优先**；战斗职责、时间推进与叙事后果边界见 [游戏循环](design/events/游戏循环.md) §1.3/§2.2 · [世界观与叙事](design/events/世界观与叙事.md) §战斗逻辑 · [剧情系统设计](design/events/剧情系统设计.md) §3.1。
...
**当前阶段**：纸面设计 + 数值模拟验证 + 引擎核心实施（战斗 / 情境 / NPC salience 已跑通，技能树与实体系统按 P 系列批次推进）。**2026-09-16 起主要精力转向故事设计与剧情生产**（[#181](https://github.com/verystrongdog/game/issues/181)）：既有战斗、动作与数值资产**全部保留**并继续被消费，但不再作为牵引方向。实时状态见 [设计框架-六维状态](design/framework/six-dimensions.md)。
...
本项目主体不是开源项目。除第三方材料及文件头明确标注的 Blender `bpy` 脚本外，项目原创内容采用**保留所有权利（All Rights Reserved）**的专有版权声明；未经书面许可，不得复制、分发、改编或商业使用。使用 Blender Python API 的公开脚本按 GPL-3.0-or-later 单独授权，具体范围以 SPDX 文件头为准。

- 完整权利条款：[`LICENSE.md`](LICENSE.md)
- 第三方素材、科研数据与许可审查状态：[`THIRD_PARTY_NOTICES.md`](THIRD_PARTY_NOTICES.md)
- 虚构作品、医学/科研与敏感内容声明：[`LEGAL.md`](LEGAL.md)
