...
| `data/` → `code/tools/` | ✅ **仅生成期** | ✏️ 2026-09-15 新增声明、**同日落地转正**（[#164](https://github.com/verystrongdog/game/issues/164)）：Blender 侧创作/校验脚本（`code/tools/`）可读 `data/` 作为生成输入（`role: generator-input`）；**首个实例** `data/hand_grip_cards.json`（[动作描述口径 §十三](../design/presentation/%E5%8A%A8%E4%BD%9C%E6%8F%8F%E8%BF%B0%E5%8F%A3%E5%BE%84.md) 手部基准卡片的机器源）：`code/tools/author_xbot_chair_grab.py --task card1` **写**、`code/tools/validate_grip_cards.py` **核**，已在 `build_data_manifest.py` 的 `GENERATOR_INPUT` 登记（manifest 101 → **102**、`generator_input` 1 → **2**）。⚠️ 运行时任何一方都不读它——`shipping: false` |
