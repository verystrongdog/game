...
> 一次动作里「**哪只脚在撑着、从哪一帧到哪一帧**」这一层状态。两个已入库术语的**共同前置**：`重心` 的判据部分要用它构出支撑多边形，`脚滑` 的 M5 取帧范围由它给出。⚠️ **本仓未建**——现有 M5 读数是在 lab 里**手选接触帧**算的；而 §2.2 已有的定帧规则（41 点采样 + `argmax`）定的是**手**到位那一帧（右拳相对髋最前伸），**不是脚的落地相位**。

⚠️ 该条目尾句「**本仓未建**」在本件落地后**已过期**（本件即那件）——但 #165 的「预期差分」明写
`data/term_registry.json` 不改，故本件**不动注册表**，只在此与证据里登记（证据 §七 未闭合第 1 条）。
⇒ 注册表条目刷新（`未建` → 已建 + 指向本件）属后续件；**R1 会在那次刷新时立刻报红**，逼着两边同步。
