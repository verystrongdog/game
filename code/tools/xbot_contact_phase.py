...
| **离线机械校验** | 无参数 / `--verify`（`--only R4` 只跑一条；纯 Python，不需要 bpy） | 对本仓库的文件跑 R1–R5（见下）——原由 `run_all_checks.py` 与 CI 编排，⚠️ 2026-09-17 两者已随全部门禁删除，现在须手动跑 | ~~门禁 `docs-integrity`~~（已删） |
...
#: ⚠️ 本件**不新拍**这个数：R2 会在每次校验里逐值核它（2026-09-17 前由 `run_all_checks.py` / CI 自动跑，现须手动）。
