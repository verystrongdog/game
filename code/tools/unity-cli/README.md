...
4. **截图必须先做帧新鲜度断言**。先改一个必然影响画面的量、连拍两张比 md5；两张相同即冻结帧，此前所有基于截图的结论全部作废。`shot.sh` 已内置「激活窗口 → 再截图」来规避，但不能替代断言。
...
---
*创建: 2026-09-12 | 更新: 2026-09-12（#137 实测：加 Error Pause 坑 + 新增 §五 确定性姿势求值）*
