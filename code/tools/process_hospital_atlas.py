...
# PDF 文件页 → 章节映射。
#
# 注意：PDF 文件页比图框内“页次”大 1；page-26.png 的图框页次为 25，
# 内容是一层平面图。这里必须按实际导出的文件名映射，不能把图框页次
# 直接当成 pdftoppm 的文件页。2026-09-17 已用 page-06、19、24—30
# 的图内标题复核这一偏移。
PAGE_SECTIONS = {
    # 文字部分（不渲染）
    **{p: "文字部分" for p in range(1, 7)},

    # 150床 方案一 (文件页 p7-11；图框页次 p6-10)
    **{p: "150床_方案一" for p in range(7, 12)},

    # 150床 方案二 (文件页 p12-15)
    **{p: "150床_方案二" for p in range(12, 16)},

    # 300床 方案一 (文件页 p16-19)
    **{p: "300床_方案一" for p in range(16, 20)},

    # 300床 方案二 (文件页 p20-24)
    **{p: "300床_方案二" for p in range(20, 25)},

    # 500床 方案一 (文件页 p25-29)
    **{p: "500床_方案一" for p in range(25, 30)},

    # 500床 方案二 (文件页 p30-33)
    **{p: "500床_方案二" for p in range(30, 34)},

    # 大样示意图（当前提取集只含文件页 p34）
    34: "大样示意图",
}

# 楼层标注（按图内标题复核；键为 PDF 文件页）
PAGE_FLOOR_LABELS = {
    7:  "一层平面图",  8:  "二层平面图",  9:  "三层平面图",
    10: "四层平面图", 11: "五层平面图",
    12: "一层平面图", 13: "二层平面图", 14: "三层平面图", 15: "四层平面图",
    16: "一层平面图", 17: "二层平面图", 18: "三层平面图", 19: "四至五层平面图",
    20: "总平面图",   21: "一层平面图", 22: "二层平面图",
    23: "三层平面图", 24: "四至五层平面图",
    25: "总平面图",   26: "一层平面图", 27: "二层平面图",
    28: "三层平面图", 29: "四至五层平面图",
    30: "总平面图",   31: "一层平面图", 32: "二层平面图", 33: "三层平面图",
    34: "门窗大样",
}
