...
//
// 本文件断言两类事实：
//   ① 镜像派生件（#150）：逐曲线非破坏、骨骼级镜像正确性、播放口径、二次镜像回原
...
//   本条沿用该口径：真正的检查在 `DerivedClipBuilder.MirrorCheck()` / `ActionLabBuilder` 的资产读取里，
...
public class ActionLabGripTests
{
    ...
    /// <summary>
    /// 镜像正确性（#150 验收标准 #3）：源件与镜像件在同一载体（X Bot）上逐采样时刻比骨骼世界位姿的**镜像**。
    /// 同时断言**对照项**：不做左右对拍的原始读数必须很大——否则这条断言是空的（对拍写错也能"通过"）。
    /// </summary>
    [Test]
    public void MirroredClip_PoseIsExactMirror_OnCarrier()
    {
        var report = (string)Call("DerivedClipBuilder", "MirrorCheck", 1f);
    ...
    /// <summary>播放口径（#150 验收标准 #4）：humanMotion、循环设置与源件一致，且**关键帧时间**证明没倒放。</summary>
    [Test]
    public void MirroredClip_IsHumanoidLoop_AndNotTimeReversed()
    {
        ...
        Assert.AreEqual("True", Field(report, "keyTimesMatch"),
            "镜像件的关键帧时间应与源件一致 / RootT 不得倒放：" + report);
    }

    /// <summary>非破坏（#150 验收标准 #2/#5）：磁盘上的派生件 == 现场重算，且两件都被覆盖。</summary>
    [Test]
    ...
    {
        var ok = (bool)Call("DerivedClipBuilder", "VerifyMirrorInvolution");
