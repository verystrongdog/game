...
1′. **待裁定：持椅挂点机制怎么处置**（owner 目视不通过之后）——三条路：① 补到"看起来像人拿"（副手 IK + 握持姿态/手指姿态匹配 + 允许碰撞参与）；② 降级为"呈现探针的已知局限"，在规格里写明它**不是**可接受的最终形态，且不得据此声称「持椅交互已落地」；③ 换机制（重新设计 §戊·2）。**在裁定之前，本文档与 `slice.md` 的「持椅交互」行都不得再声称该层"已落地"。**
