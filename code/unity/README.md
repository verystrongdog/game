...
> 🔧 **2026-09-14：本条产出的镜像件已退役**（owner 裁定）——格挡变体的姿势来源改用 `2hand Idle`，镜像件失去消费者，`DeriveMirrored` / `FixRootTranslation` / `MirrorCheck` / `VerifyMirrorInvolution` 连同资产一并删除（提交 `95aa95e`，`DerivedClipBuilder.cs` 623 → 231 行）。**下面的历史读数与判据全部保留**（那三条取值口径仍然有效，将来再做 L/R 重映射从这里起步）；`run_tests` 50/50、`validate_unity_assets` A3 悬空 0。
...
## 二·Q、Blender 回导样本与逐帧调试 lab（[#156](https://github.com/verystrongdog/game/issues/156)，2026-09-14）

> 把「Blender 里做的动作」变成「Unity 里能逐帧看、且复用得了」的样本。**口径正典**：
> [Blender动作制作管线](../../design/presentation/Blender%E5%8A%A8%E4%BD%9C%E5%88%B6%E4%BD%9C%E7%AE%A1%E7%BA%BF.md)（与 [动画处理能力对照实验](../../design/presentation/动画处理能力对照实验.md) 同为两线共用口径）。
...
| 断言 | `Assets/Tests/PlayMode/BlenderAnimationDebugTests.cs`（7 条：Rig 2 · 采样 1 · 控制 2 · 导出物形状 1 · 资产身份 1） |
...
| 判据 | 读数 |
|---|---|
| 母版 | 65 骨 + **13 根 `CTRL_*`**（`use_deform=False`）· 单一根 `mixamorig:Hips` · 探针动作 `RigRoundTripProbe`（1/21/41/61 帧 @30fps）· 输入 `X Bot.fbx` sha256 运行前后一致 |
| 控制骨真的驱动身体 | 逐根摆 25° → 目标骨尖位移最小 **35.61 mm**（阈值 20） |
| 探针非退化 | 读数点最大位移 **820.21 mm**；逐点 Hips 42.05 / 左手 746.38 / 右手 820.21 / 左脚 608.26 / 右脚 601.19 |
| 导出物形状 | 骨 **65** · `CTRL_` **0** · 根 `['mixamorig:Hips']` · 网格 **0**（animation-only）· 684 044 字节 |
| 烘焙 / 回导保真（Blender 侧） | **0.0011 mm** / **0.0014 mm**（帧号 offset 0）· Rest Pose 最大元素差 9.2e-05 |
| Unity 导入 | `animationType=3` · `avatarSetup=1` · `animationCompression=0` · `clip.humanMotion=True` · 时长 2.0000 s @30fps · 三条导入告警**串皆空** · `RigRoundTripProbeAvatar isValid=True isHuman=True` |
| 逐帧读数 | 同一帧重复三次采样 `ReadoutLine()` **逐字节相同**；帧间最大位移 Hips **16.94 mm** / 左手 **677.06 mm** / 左脚 **550.98 mm** |
| 骨架完整性（目视的数值代理） | 上臂/前臂/大腿/小腿骨长量程 ≤ **0.0004 mm**（无比例跳变）· 胸骨 `up.y` 恒 **0.9837–0.9891**（无骨架翻转）· 导出物 0 个控制骨节点（无多余节点） |
| 门禁 `unity` | `run_tests` **63 项：62 过 / 0 败 / 1 跳过**（56 → 63，新增 7 条全过；跳过的是既有的 `ActionLabGuardVariantTests`） |
...
│   │   ├── BlenderAnimationDebugBuilder.cs # Blender 回导样本：Humanoid 导入设置纠正 + 幂等生成 Assets/Temp/ lab（#156）
...
│   │   └── BlenderAnimationDebugger.cs # Blender 回导 clip 逐帧调试：播放/暂停/单帧/复位 + Hips·双手·双脚读数（#156）
...
│       ├── BlenderAnimationDebugTests.cs # Blender 回导：Rig 2 条 + 采样稳定性 1 条 + 调试控制 2 条 + 导出物形状 1 条 + 资产身份 1 条（#156）
...
> 场景与控制器：`Assets/Scenes/ActionLab.unity` + `Assets/ActionLab/ActionLab.controller` 已入库（基准场景，#136）；**`Assets/Scenes/AnimationRiggingLab.unity` 也已入库**（#157 的显式例外：约束只认已序列化场景，断言须按名 `LoadScene`——理由见 §二·P 与 §二·H）；**`Assets/Animations/Blender/RigRoundTripProbe.fbx` 入库**（#156 的探针样本），而它驱动的 lab 场景与 controller 是**生成物**、只落 `Assets/Temp/`（§二·Q）；其余 lab 场景由 builder 菜单重建，不入库。
