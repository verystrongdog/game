...
| 逐帧控制 | 空格 播放/暂停 · ←/→ 单帧 · `R` 回首帧；HUD 显示 `clip / 帧号 / Hips·双手·双脚读数`；**连续播放在末帧接回首帧**（否则停住看不见） |
