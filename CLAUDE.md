...
| 意识外显效应 | 世界观核心——意识内容在月光下外显为物理效力，多版本基态对单子呈现（观察=单子取版本） | 不是比喻，是 game mechanic（原名"观察者效应"，2026-08-24 #38 改名） |
...
**skill 文件小节编号豁免（2026-08-16 新增）：** `.claude/skills/*/SKILL.md` 与 `.agents/skills/*/SKILL.md` 属于**可执行指令文档**而非设计文档，其小节标题**豁免**中文数字要求，可用描述性标题（如 `## 执行流程`、`## 输出格式`），与 `review-plan`、`task-checker` 现有惯例一致。豁免范围仅限 `SKILL.md` 本体；skill 引用的子文档/规范文档仍须遵守中文数字小节编号。
...
> **AI 必须打断用户对话中的模糊表述，要求用户亲自用数学语言重述，达标后才继续。此规范全程生效，无场景豁免。标准锚定：数学专业论文的表述规范。**

**触发范围**：全程（grilling、新设计、代码/文档需求、参数修改、C# 教学、闲聊中出现的可量化语义，一律严谨）。

**执行机制**（顺序执行）：
1. **打断**：检测到模糊表述 → 立即停止当前话题推进。
2. **指出**：用固定句式指出具体模糊点（引用触发词 + 所在句子），允许给触发词类型提示（阈值模糊/枚举未受控/量词缺失/符号未定义），但**不得代写、不得给改写方向、不得给示例**。
3. **等待重述**：用户亲自重述（这是思维训练本体，AI 代写会剥夺训练）。
...
**可保留自然语言**：纯意图/动机/情绪表达（无量化成分）不受限，如「我觉得这个设计让玩家困惑」。
...
(c) 两个已有术语的边界被重新划分（如 DFS 集体退相干 vs Zeno 子空间 vs 意识外显耦合——三个退相干抑制机制的层面分离）
